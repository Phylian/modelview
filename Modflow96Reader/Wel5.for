...
c      WRITE(IOUT,1)IN
c    1 FORMAT(1X,/1X,'WEL5 -- WELL PACKAGE, VERSION 5, 9/1/93',
...
C2------CELL-BY-CELL FLOW TERMS.
	...
	if (ierror.ne.0) goto 20
      IF(IFREFM.EQ.0) THEN
         READ(LINE,'(2I10)',IOSTAT=ierror) MXWELL,IWELCB
  	   if (ierror.ne.0) goto 20
         LLOC=21
      ELSE
         LLOC=1
      ...
      END IF
c      WRITE(IOUT,3) MXWELL
c    3 FORMAT(1X,'MAXIMUM OF',I5,' WELLS')
c      IF(IWELCB.LT.0) WRITE(IOUT,7)
c    7 FORMAT(1X,'CELL-BY-CELL FLOWS WILL BE PRINTED WHEN ICBCFL NOT 0')
c      IF(IWELCB.GT.0) WRITE(IOUT,8) IWELCB
...
c            WRITE(IOUT,12) WELAUX(NAUX)
c   12       FORMAT(1X,'AUXILIARY WELL PARAMETER: ',A)
         END IF
         GO TO 10
      END IF
...
c      WRITE(IOUT,14) ISP
c   14 FORMAT(1X,I10,' ELEMENTS IN X ARRAY ARE USED BY WEL')
c      ISUM1=ISUM-1
...
c      WRITE(IOUT,6)
c    6 FORMAT(1X,/1X,'REUSING WELLS FROM LAST STRESS PERIOD')
      RETURN
C
...
c      WRITE(IOUT,99) NWELLS,MXWELL
c   99 FORMAT(1X,/1X,'NWELLS(',I4,') IS GREATER THAN MXWELL(',I4,')')
c     to do: error handling
      return      
c      STOP
C
C3------PRINT NUMBER OF WELLS IN CURRENT STRESS PERIOD.
  100 continue
c  100 WRITE (IOUT,101) NWELLS
c  101 FORMAT(1X,//1X,I5,' WELLS')
C
...
C5------READ AND PRINT DATA FOR EACH WELL.
      ...
      IF(NAUX.GT.0) THEN
c         WRITE(IOUT,103) (WELAUX(JJ),JJ=1,NAUX)
...
c         WRITE(IOUT,104)
      END IF
c  103 FORMAT(1X,/
c     1       1X,'LAYER   ROW   COL   STRESS RATE   WELL NO.',:5(2X,A))
c  104 FORMAT(1X,42('-'),5A)
      DO 250 II=1,NWELLS
...
C5B-----READ ANY AUXILIARY DATA WITH FREE FORMAT, AND PRINT ALL VALUES.
  ...
  110    CONTINUE
c         WRITE (IOUT,115) K,I,J,WELL(4,II),II,
c     1         (WELL(JJ,II),JJ=5,MAXAUX)
      ELSE
c         WRITE (IOUT,115) K,I,J,WELL(4,II),II
      END IF
c  115 FORMAT(1X,I4,I7,I6,G15.5,I7,:5(2X,G16.5))
      WELL(1,II)=K
      WELL(2,II)=I
      WELL(3,II)=J
  250 CONTINUE
