...
c      WRITE(IOUT,1)IN
c    1 FORMAT(1X,/1X,'RIV5 -- RIVER PACKAGE, VERSION 5, 9/1/93',
...
C2------CELL-BY-CELL FLOW TERMS.
	...
	IF (IERROR.ne.0) GOTO 20
      IF(IFREFM.EQ.0) THEN
         READ(LINE,'(2I10)',IOSTAT=IERROR) MXRIVR,IRIVCB
 	   IF (IERROR.ne.0) GOTO 20
         LLOC=21
      ELSE
         LLOC=1
      ...
      END IF
c      WRITE(IOUT,3) MXRIVR
c    3 FORMAT(1X,'MAXIMUM OF',I5,' RIVER REACHES')
c      IF(IRIVCB.LT.0) WRITE(IOUT,7)
c    7 FORMAT(1X,'CELL-BY-CELL FLOWS WILL BE PRINTED WHEN ICBCFL NOT 0')
c      IF(IRIVCB.GT.0) WRITE(IOUT,8) IRIVCB
c    8 FORMAT(1X,'CELL-BY-CELL FLOWS WILL BE SAVED ON UNIT',I3)
C
...
c            WRITE(IOUT,12) RIVAUX(NAUX)
c   12       FORMAT(1X,'AUXILIARY RIVER PARAMETER: ',A)
         END IF
         GO TO 10
      END IF
...
c      WRITE (IOUT,14)ISP
c   14 FORMAT(1X,I10,' ELEMENTS IN X ARRAY ARE USED BY RIV')
      ISUM1=ISUM-1
c      WRITE(IOUT,15)ISUM1,LENX
c   15 FORMAT(1X,I10,' ELEMENTS OF X ARRAY USED OUT OF ',I10)
c      IF(ISUM1.GT.LENX) WRITE(IOUT,16)
c   16 FORMAT(1X,'   ***X ARRAY MUST BE DIMENSIONED LARGER***')
C
C6------RETURN.
20    continue
      RETURN
...
C1------READ ITMP (NUMBER OF RIVER REACHES OR FLAG TO REUSE DATA).
      IF(IFREFM.EQ.0) THEN
         READ(IN,'(I10)',IOSTAT=ierror) ITMP
      ELSE
         READ(IN,*,IOSTAT=ierror) ITMP
      END IF
...
c      WRITE(IOUT,7)
c    7 FORMAT(1X,/1X,'REUSING RIVER REACHES FROM LAST STRESS PERIOD')
      GO TO 260
C
...
c      WRITE(IOUT,99)NRIVER,MXRIVR
c   99 FORMAT(1X,/1X,'NRIVER(',I4,') IS GREATER THAN MXRIVR(',I4,')')
C
C4A-----ABNORMAL STOP.
c      STOP
      ierror=20 
...
C5------PRINT NUMBER OF RIVER REACHES IN THIS STRESS PERIOD.
  100 CONTINUE
c  100 WRITE(IOUT,101)NRIVER
c  101 FORMAT(1X,//1X,I5,' RIVER REACHES')
C
...
c      IF(NAUX.GT.0) THEN
c         WRITE(IOUT,103) (RIVAUX(JJ),JJ=1,NAUX)
c         WRITE(IOUT,104) ('------------------',JJ=1,NAUX)
c      ELSE
c         WRITE(IOUT,103)
c         WRITE(IOUT,104)
c      END IF
c  103 FORMAT(1X,/1X,'LAYER   ROW   COL     STAGE   CONDUCTANCE   ',
c     1           'BOT. ELEV.  REACH NO.',:5(2X,A))
c  104 FORMAT(1X,65('-'),5A)
      DO 250 II=1,NRIVER
C7A-----READ THE REQUIRED DATA WITH FIXED OR FREE FORMAT.
      READ(IN,'(A)',IOSTAT=ierror) LINE
      IF (IERROR.NE.0) goto 255
      IF(IFREFM.EQ.0) THEN
         READ(LINE,'(3I10,3F10.0)',IOSTAT=ierror) 
	1     K,I,J,(RIVR(JJ,II),JJ=4,6)
         IF (IERROR.NE.0) goto 255
         LLOC=61
      ELSE
         LLOC=1
...
C7B-----READ ANY AUXILIARY DATA WITH FREE FORMAT, AND PRINT ALL VALUES.
  ...
  110    CONTINUE
c         WRITE (IOUT,115) K,I,J,RIVR(4,II),RIVR(5,II),RIVR(6,II),II,
c     1         (RIVR(JJ,II),JJ=7,MAXAUX)
c      ELSE
c         WRITE (IOUT,115) K,I,J,RIVR(4,II),RIVR(5,II),RIVR(6,II),II
      END IF
c  115 FORMAT(1X,I4,I7,I6,G13.4,G12.4,G12.4,I8,:5(2X,G16.5))
      RIVR(1,II)=K
      RIVR(2,II)=I
      RIVR(3,II)=J
  250 CONTINUE
