...
c      WRITE(IOUT,1)IN
c    1 FORMAT(1H0,'RES1 -- RESERVOIR PACKAGE, VERSION 1, 1/15/95',
c     1' INPUT READ FROM UNIT',I3)
C
C2------READ & PRINT NUMBER OF RESERVOIRS AND FLAGS FOR
C2------RESERVOIR OPTIONS
      READ(IN,2,IOSTAT=ierror) NRES,IRESCB,NRESOP,IRESPT,NPTS
    2 FORMAT(5I10)
...
C2A-----PRINT VALUE
      IF(NRES.GT.0) THEN
c       WRITE(IOUT,6) NRES
c    6 FORMAT(1X,'TOTAL NUMBER OF RESERVOIRS: ',I3)
      ELSE
c       WRITE (IOUT,7)
c    7 FORMAT(1X,'ABORTING, NUMBER OF RESERVOIRS LESS THAN 1...')
        ierror=20
        goto 400
      ENDIF 
C
C2B-----CHECK FLAG FOR CELL-BY-CELL OUTPUT, PRINT VALUE
c      IF(IRESCB.GT.0) WRITE(IOUT,10) IRESCB
c 10   FORMAT(1X,'CELL-BY-CELL FLOWS WILL BE RECORDED ON UNIT',I3)
...
c      WRITE(IOUT,8)
c    8 FORMAT(1X,'ILLEGAL OPTION CODE. SIMULATION ABORTING')
        ierror=20
        goto 400
C
C2C2----IF OPTION IS LEGAL PRINT OPTION CODE.
 200  IRK=ISUM
c      IF(NRESOP.EQ.1) WRITE(IOUT,201)
c  201 FORMAT(1X,'OPTION 1 -- RESERVOIR CONNECTED TO TOP LAYER')
c      IF(NRESOP.EQ.2) WRITE(IOUT,202)
c  202 FORMAT(1X,'OPTION 2 -- RESERVOIR CONNECTED TO ONE SPECIFIED',
c     1        ' NODE IN EACH VERTICAL COLUMN')
c      IF(NRESOP.EQ.3) WRITE(IOUT,203)
c  203 FORMAT(1X,'OPTION 3 -- RESERVOIR CONNECTED TO HIGHEST',
c     1        ' ACTIVE NODE IN EACH VERTICAL COLUMN')
C
C2D-----PRINT VALUE FOR RESERVIOR PRINT OPTION FLAG
c      IF(IRESPT.GT.0) WRITE(IOUT,14) 
c 14   FORMAT(1X,'RESERVOIR HEADS, AREAS, AND VOLUMES ',
c     1 'WILL BE PRINTED EACH TIME STEP')
C2E-----PRINT NUMBER OF POINTS TO BE USED IN CALCULATING TABLE
C2E-----OF RESERVOIR STAGE VS. AREA AND VOLUME
c      IF(NPTS.LT.1) THEN
c       WRITE(IOUT,*) ' Table of reservoir areas and volumes ',
c     1 'will not be calculated.'
c      ELSE
c       WRITE(IOUT,9) NPTS
c 9     FORMAT(I5,' points will be used in constructing table of ',
c     1  'reservoir areas and volumes.')
c      ENDIF
C
...
c      WRITE (IOUT,30)ISP
c 30   FORMAT(1X,I8,' ELEMENTS IN X ARRAY ARE USED FOR RESERVOIRS')
      ISUM1=ISUM-1
c      WRITE(IOUT,40)ISUM1,LENX
c 40   FORMAT(1X,I8,' ELEMENTS OF X ARRAY USED OUT OF',I8)
c      IF(ISUM1.GT.LENX) WRITE(IOUT,50)
c 50   FORMAT(1X,'   ***X ARRAY MUST BE DIMENSIONED LARGER***')
C
C5------RETURN
400   CONTINUE
      RETURN
      END
      SUBROUTINE RES1RP1(IRES,IRESL,BRES,CRES,BBRES,IBOUND,
     1                   NRES,NRESOP,NPTS,NCOL,NROW,NLAY,
     2                   IN,IOUT,NCELL,ierror)
C
C
...
C     ------------------------------------------------------------------
     ...
     1 CRES(NCOL,NROW),BBRES(NCOL,NROW),
...
C
C      IF(.NOT.FIRST) GO TO 70
C      FIRST=.FALSE.
      KK=1
      CALL U2DINT(IRES,ANAME(1,1),NROW,NCOL,KK,IN,IOUT,ierror)
...
c rbw end
   ...
   20 CONTINUE
   30 CONTINUE
C      IF(NCELL.GT.0) THEN
C       WRITE(IOUT,32) N,NCELL
C   32  FORMAT(1X,'NUMBER OF CELLS IN RESERVOIR ',I2,':',I6)
C      ELSE
C       WRITE(IOUT,34)
C   34 FORMAT(1X,'NO ACTIVE CELLS FOUND FOR RESERVOIR ',I2,'.',
C     1 '  ABORTING...')
C      ENDIF
   36 CONTINUE
C
C3------IF NRESOP=2 THEN A LAYER INDICATOR ARRAY IS NEEDED.
      ...
      CALL U2DINT(IRESL,ANAME(1,2),NROW,NCOL,0,IN,IOUT,ierror)
	if (ierror.NE.0) return
...
C       CRES(J,I)=CRES(J,I)*DELC(I)*DELR(J)/BBRES(J,I)
...
C5A-----FIND MAX AND MIN BOTTOM ELEVATION
   ...
   44 CONTINUE
C5B-----CONSTRUCT TABLE
C      WRITE(IOUT,46) N,ELMIN
C   46 FORMAT(1X,'STAGE-VOLUME TABLE FOR RESERVOIR',I2,/,6X,
C     1 'STAGE       VOLUME         AREA',/,
C     2 3X,36('-'),/,1X,G10.5,2(11X,'0.0'))
...
C      IF(STAGE.GT.BRES(J,I))THEN
C       AREA=DELR(J)*DELC(I)
C       TAREA=TAREA+AREA
C       VOL=VOL+AREA*(STAGE-BRES(J,I))
C      ENDIF
   48 CONTINUE
   50 CONTINUE
C      WRITE(IOUT,54) STAGE,VOL,TAREA
C   54 FORMAT(1X,G10.5,2G14.5)
   56 CONTINUE
C      WRITE(IOUT,58)
C   58 FORMAT(1X,' ')
   60 CONTINUE
...
C8------RETURN
      RETURN
      END
      SUBROUTINE RES1RP2(IRES,IRESL,BRES,CRES,BBRES,HRESSE,IBOUND,
