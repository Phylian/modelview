     ...
     2                   LCTBAR,LCTRIB,LCIVAR,LCFGAR,ierror)
C                                                                      C
...
C      WRITE(IOUT,1) IN
C    1 FORMAT(1H0,'STRM -- STREAM PACKAGE, VERSION 2, 12/18/90 ',
C     1'INPUT READ FROM UNIT',I3)
      NSTREM=0
C                                                                      C
C2------ READ MXSTRM, NSS, NTRIB, ISTCB1, AND ISTCB2.                  C
  100 READ(IN,3,IOSTAT=ierror)MXSTRM,NSS,NTRIB,NDIV,ICALC,CONST,ISTCB1,
     1  ISTCB2
    3 FORMAT(5I10,F10.0,2I10)
	if (ierror.NE.0) goto 300
      IF(MXSTRM.LT.0)MXSTRM=0
      IF(NSS.LT.0)NSS=0
C      WRITE(IOUT,4)MXSTRM,NSS,NTRIB
C    4 FORMAT(1H ,'MAXIMUM OF',I5,' STREAM NODES'//1X,'NUMBER OF STREAM S
C     1EGMENTS IS ',I5//1X,'NUMBER OF STREAM TRIBUTARIES IS ',I5//)
C      IF(NDIV.GT.0) WRITE(IOUT,5)
C    5 FORMAT(1H ,'DIVERSIONS FROM STREAMS HAVE BEEN SPECIFIED')
C      IF(ICALC.GT.0) WRITE(IOUT,6) CONST
C    6 FORMAT(1H ,'STREAM STAGES WILL BE CALCULATED USING A CONSTANT OF
C     1',F10.4)
C      IF(ISTCB1.GT.0) WRITE(IOUT,7) ISTCB1,ISTCB2
C    7 FORMAT(1X,'CELL BUDGETS WILL BE SAVED ON UNITS',I3,'AND',I3)
C                                                                      C
...
C      WRITE (IOUT,8)ISP
C    8 FORMAT(1X,I8,' ELEMENTS IN X ARRAY ARE USED FOR STREAMS')
      ISUM1=ISUM-1
C      WRITE(IOUT,9)ISUM1,LENX
C    9 FORMAT(1X,I8,' ELEMENTS OF X ARRAY USED OUT OF',I7)
C      IF(ISUM1.GT.LENX) WRITE(IOUT,10)
C   10 FORMAT(1X,'   ***X ARRAY MUST BE DIMENSIONED LARGER***')
C                                                                      C
C10-----RETURN. 
300   CONTINUE                                                       
      RETURN
     ...
     1                   NSS,NTRIB,IDIVAR,ICALC,IPTFLG,ierror)
C                                                                      C
C                                                                      C
...
C1B-----READ ITMP(NUMBER OF STREAM CELLS OR FLAG TO REUSE DATA).       C
      READ(IN,1,IOSTAT=ierror)ITMP,IRDFLG,IPTFLG
    1 FORMAT(3I10)
      IF (IERROR.NE.0) goto 370
C                                                                      C
C2A-----IF ITMP <0 THEN REUSE DATA FROM LAST STRESS PERIOD.            C
      IF(ITMP.GE.0)GO TO 50
c      WRITE(IOUT,2)
c    2 FORMAT(1H0,'REUSING STREAM NODES FROM LAST STRESS PERIOD')
      RETURN
C                                                                      C
...
c      WRITE(IOUT,99)NSTREM,MXSTRM
c   99 FORMAT(1H0,'NSTREM(',I4,') IS GREATER THAN MXSTRM(',I4,')')
c      STOP
      ierror=20 
      goto 370
C                                                                      C
C3B-----PRINT NUMBER OF STREAM CELLS IN THIS STRESS PERIOD.            C
  100 CONTINUE
c  100 IF(IRDFLG.EQ.0) WRITE(IOUT,3)NSTREM
c    3 FORMAT(1H0,//1X,I5,' STREAM NODES')
C                                                                      C
C4------IF THERE ARE NO STREAM CELLS THEN RETURN.                      C
      IF(NSTREM.EQ.0) RETURN
C                                                                      C
C5------READ AND PRINT DATA FOR EACH STREAM CELL.                      C
c      IF(IRDFLG.EQ.0) WRITE(IOUT,4)
c    4 FORMAT(1H ,3X,'LAYER   ROW    COL    SEGMENT   REACH   STREAMFLOW
c     1     STREAM    STREAMBED     STREAMBED BOT  STREAMBED TOP',/27X,
c     2'NUMBER   NUMBER                   STAGE   CONDUCTANCE      ELEVAT
c     3ION      ELEVATION',/3X,110('-'))
      DO 250 II=1,NSTREM
      READ(IN,5,IOSTAT=ierror)K,I,J,ISTRM(4,II),ISTRM(5,II),STRM(1,II),
	1 STRM(2,II), STRM(3,II),STRM(4,II),STRM(5,II)
    5 FORMAT(5I5,F15.0,4F10.0)
      IF (IERROR.NE.0) goto 370
c      IF(IRDFLG.EQ.0) WRITE(IOUT,6)K,I,J,ISTRM(4,II),ISTRM(5,II),
c     1STRM(1,II),STRM(2,II),STRM(3,II),STRM(4,II),STRM(5,II)
c    6 FORMAT(1X,3X,I4,2I7,2I9,7X,G11.4,G12.4,G11.4,4X,2G13.4)
      ISTRM(1,II)=K
      ISTRM(2,II)=I
      ISTRM(3,II)=J
  250 CONTINUE
...
c      IF(IRDFLG.EQ.0) WRITE(IOUT,7)
c    7 FORMAT(1H0,3X,'LAYER',3X,'ROW',4X,'COL   ',' SEGMENT',3X,
c     1'REACH',8X,'STREAM',13X,'STREAM',10X,'ROUGH',/27X,'NUMBER',3X,
c     2 'NUMBER',8X,'WIDTH',14X,'SLOPE',10X,'COEF.',/3X,110('-'))
      DO 280 II=1,NSTREM
      READ(IN,8,IOSTAT=ierror) STRM(6,II),STRM(7,II),STRM(8,II)
    8 FORMAT(3F10.0)
      IF (IERROR.NE.0) goto 370
c      IF(IRDFLG.EQ.0) WRITE(IOUT,9)ISTRM(1,II),ISTRM(2,II),ISTRM(3,II),
c     1ISTRM(4,II),ISTRM(5,II),STRM(6,II),STRM(7,II),STRM(8,II)
c    9 FORMAT(4X,I4,2I7,2I9,7X,G12.4,4X,G13.4,4X,G12.4)
  280 CONTINUE
C                                                                      C
...
c      IF(IRDFLG.EQ.0) WRITE(IOUT,10)NTRIB
c   10 FORMAT(1H0,30X,'MAXIMUM NUMBER OF TRIBUTARY STREAMS IS ',I5,//1X,
c     1 20X,'STREAM SEGMENT',15X,'TRIBUTARY STREAM SEGMENT NUMBERS')
      DO 340 IK=1,NSS
      READ(IN,11,IOSTAT=ierror) (ITRBAR(IK,JK),JK=1,NTRIB)
   11 FORMAT(10I5)
      IF (IERROR.NE.0) goto 370
c      IF(IRDFLG.EQ.0) WRITE(IOUT,12)IK,(ITRBAR(IK,JK),JK=1,NTRIB)
c   12 FORMAT(20X,I5,20X,10I5)
  340 CONTINUE
C                                                                      C
C10----READ AND PRINT DIVERSION SEGMENTS NUMBERS.                      C
  343 IF(NDIV.LE.0) GO TO 350
c      IF(IRDFLG.EQ.0) WRITE(IOUT,13)
c   13 FORMAT(1H0,10X,'DIVERSION SEGMENT NUMBER',10X,
c     1       'UPSTREAM SEGMENT NUMBER')
      DO 345 IK=1,NSS
      READ(IN,14,IOSTAT=ierror) IDIVAR(IK)
   14 FORMAT(I10)
      IF (IERROR.NE.0) goto 370
c      IF(IRDFLG.EQ.0) WRITE(IOUT,15) IK,IDIVAR(IK)
c   15 FORMAT(20X,I5,28X,I5)
  345 CONTINUE
C                                                                      C
