...
C      WRITE(IOUT,1)IN
C1     FORMAT(1X,/1X,'GHB5 -- GHB PACKAGE, VERSION 5, 9/1/93',
...
C2------CELL-BY-CELL FLOW TERMS.
	...
	IF (IERROR.NE.0) goto 20
      IF(IFREFM.EQ.0) THEN
         READ(LINE,'(2I10)',IOSTAT=ierror) MXBND,IGHBCB
  	   IF (IERROR.NE.0) goto 20
         LLOC=21
      ELSE
         LLOC=1
      ...
      END IF
c      WRITE(IOUT,3) MXBND
c    3 FORMAT(1X,'MAXIMUM OF',I5,' HEAD-DEPENDENT BOUNDARY NODES')
c      IF(IGHBCB.LT.0) WRITE(IOUT,7)
c    7 FORMAT(1X,'CELL-BY-CELL FLOWS WILL BE PRINTED WHEN ICBCFL NOT 0')
c      IF(IGHBCB.GT.0) WRITE(IOUT,8) IGHBCB
...
c            WRITE(IOUT,12) GHBAUX(NAUX)
c   12       FORMAT(1X,'AUXILIARY BOUNDARY PARAMETER: ',A)
         END IF
         GO TO 10
      END IF
...
c      WRITE(IOUT,14) ISP
c   14 FORMAT(1X,I10,' ELEMENTS IN X ARRAY ARE USED BY GHB')
      ISUM1=ISUM-1
...
c      WRITE(IOUT,7)
c    7 FORMAT(1X,/1X,'REUSING HEAD-DEPENDENT BOUNDS FROM LAST STRESS',
c     1      ' PERIOD')
      GO TO 260
C
...
c      WRITE(IOUT,99) NBOUND,MXBND
c   99 FORMAT(1X,/1X,'NBOUND(',I4,') IS GREATER THAN MXBND(',I4,')')
C
C4A-----ABNORMAL STOP.
c      STOP
      ierror=20 
...
C5------PRINT # OF GENERAL HEAD BOUNDS THIS STRESS PERIOD.
  100 CONTINUE
c  100 WRITE(IOUT,101) NBOUND
c  101 FORMAT(1X,//1X,I5,' HEAD-DEPENDENT BOUNDARY NODES')
C
...
c      IF(NAUX.GT.0) THEN
c         WRITE(IOUT,103) (GHBAUX(JJ),JJ=1,NAUX)
c         WRITE(IOUT,104) ('------------------',JJ=1,NAUX)
c      ELSE
c         WRITE(IOUT,103)
c         WRITE(IOUT,104)
c      END IF
c  103 FORMAT(1X,/1X,'LAYER   ROW   COL   ELEVATION   CONDUCTANCE   ',
c     1           'BOUND NO.',:5(2X,A))
c  104 FORMAT(1X,55('-'),5A)
      DO 250 II=1,NBOUND
C7A-----READ THE REQUIRED DATA WITH FIXED OR FREE FORMAT.
      READ(IN,'(A)',IOSTAT=ierror) LINE
      IF (IERROR.NE.0) goto 255
      IF(IFREFM.EQ.0) THEN
         READ(LINE,'(3I10,2F10.0)',IOSTAT=ierror) 
	1     K,I,J,(BNDS(JJ,II),JJ=4,5)
         IF (IERROR.NE.0) goto 255
         LLOC=51
      ELSE
         LLOC=1
...
C7B-----READ ANY AUXILIARY DATA WITH FREE FORMAT, AND PRINT ALL VALUES.
  ...
  110    CONTINUE
c         WRITE (IOUT,115) K,I,J,BNDS(4,II),BNDS(5,II),II,
c     1         (BNDS(JJ,II),JJ=6,MAXAUX)
c      ELSE
c         WRITE (IOUT,115) K,I,J,BNDS(4,II),BNDS(5,II),II
      END IF
c  115 FORMAT(1X,I4,I7,I6,G13.4,G14.4,I8,:5(2X,G16.5))
      BNDS(1,II)=K
      BNDS(2,II)=I
      BNDS(3,II)=J
  250 CONTINUE
