...
C      WRITE(IOUT,1)IN
C    1 FORMAT(1X,/1X,'DRN5 -- DRAIN PACKAGE, VERSION 5, 9/1/93',
...
C2------CELL-BY-CELL FLOW TERMS.
	...
	IF (IERROR.NE.0) goto 20
      IF(IFREFM.EQ.0) THEN
         READ(LINE,'(2I10)',IOSTAT=ierror) MXDRN,IDRNCB
         IF (IERROR.NE.0) goto 20
         LLOC=21
      ELSE
         LLOC=1
      ...
      END IF
c      WRITE(IOUT,3) MXDRN
C    3 FORMAT(1X,'MAXIMUM OF',I5,' DRAINS')
...
C   12       FORMAT(1X,'AUXILIARY DRAIN PARAMETER: ',A)
         END IF
         GO TO 10
      END IF
...
C      WRITE(IOUT,14) ISP
C   14 FORMAT(1X,I10,' ELEMENTS IN X ARRAY ARE USED BY DRN')
      ISUM1=ISUM-1
...
c      WRITE(IOUT,7)
c    7 FORMAT(1X,/1X,'REUSING DRAINS FROM LAST STRESS PERIOD')
      RETURN
C
...
c      WRITE(IOUT,99) NDRAIN,MXDRN
c   99 FORMAT(1X,/1X,'NDRAIN(',I4,') IS GREATER THAN MXDRN(',I4,')')
c      STOP
      ierror=20 
      goto 255
C
C5------PRINT NUMBER OF DRAINS IN THIS STRESS PERIOD.
  100 CONTINUE
c  100 WRITE(IOUT,101) NDRAIN
c  101 FORMAT(1X,//1X,I5,' DRAINS')
C
...
c      IF(NAUX.GT.0) THEN
c         WRITE(IOUT,103) (DRNAUX(JJ),JJ=1,NAUX)
...
C7B-----READ ANY AUXILIARY DATA WITH FREE FORMAT, AND PRINT ALL VALUES.
  ...
  110    CONTINUE
c         WRITE (IOUT,115) K,I,J,DRAI(4,II),DRAI(5,II),II,
c     1         (DRAI(JJ,II),JJ=6,MAXAUX)
c      ELSE
c         WRITE (IOUT,115) K,I,J,DRAI(4,II),DRAI(5,II),II
      END IF
