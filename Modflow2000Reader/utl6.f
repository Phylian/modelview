...
C
   ...
   20 RETURN
      END
C=======================================================================
c###      SUBROUTINE UNOCONV(D,OBSNAM,H,HOBS,IOUT,IP,IPAR,IPR,KPER,
c###     &                   KSTP,LN,MPR,ND,NDMH,NH,NIPR,PRM,R,RSQ,
c###     &                   RSQP,WP,WTPS,WT,WTQ,WTQS,NPLIST,MPRAR,
c###     &                   IPRAR,OUTNAM,IPLOT,EQNAM,NAMES,IPLPTR,NDMHAR,
c###     &                   NQTDR,NQTRV,NQTGB,NQTST,NQTCH,IOWTQCH,IOWTQDR,
c###     &                   IOWTQRV,IOWTQGB,IOWTQST,LCOBBAS,LCOBDRN,
c###     &                   LCOBRIV,LCOBGHB,LCOBSTR,LCOBCHD,LCOBADV,SSGF,
c###     &                   SSDR,SSRV,SSGB,SSST,SSAD,SSCH,SSPI,SSTO,ITMXP,
c###     &                   IPES,BPRI,ITERP,IERR,IERRU,NTT2,LCOBDRT,SSDT,
c###     &                   NQTDT,IOWTQDT,NRSO,NPOST,NNEGT,NRUNS,NQTSF,
c###     &                   IOWTQSF,LCOBSFR,SSSF,KTDIM,NHT,OTIME)
c###C-----VERSION 20010329 ERB
c###C     ******************************************************************
c###C     BECAUSE CONVERGENCE CRITERIA HAVE NOT BEEN MET,
c###C     PRINT WARNINGS ABOUT TABLES OF WEIGHTED RESIDUALS AND PRIOR
c###C     ESTIMATES AND INVOKE SUBROUTINE TO PRINT TABLES
c###C     ******************************************************************
c###C        SPECIFICATIONS:
c###C     ------------------------------------------------------------------
c###      INTEGER IOUT, IP, IPAR, IPR, KPER, KSTP, LN, MPR, ND,
c###     &        NDMH, NH, NHT, NIPR
c###      REAL BPRI, D, H, HOBS, PRM, R, RSQ, RSQP, WP, WT, WTQ, WTQS
c###      INTEGER IPLOT(ND+IPR+MPR), IPLPTR(ND+IPR+MPR)
c###      CHARACTER*12 OBSNAM
c###      CHARACTER*10 EQNAM(MPRAR)
c###      CHARACTER*12 NAMES(ND+IPR+MPR)
c###      CHARACTER*200 OUTNAM
c###      DIMENSION D(1), OBSNAM(ND), H(ND), HOBS(ND),
c###     &          LN(NPLIST), NIPR(IPRAR), PRM(NPLIST+1,MPRAR), R(1),
c###     &          WP(MPRAR), WT(ND), SSGF(ITMXP+1), SSDR(ITMXP+1),
c###     &          SSRV(ITMXP+1), SSGB(ITMXP+1), SSST(ITMXP+1),
c###     &          SSAD(ITMXP+1), SSCH(ITMXP+1), SSPI(ITMXP+1),
c###     &          SSTO(ITMXP+1), BPRI(IPRAR)
c###      DIMENSION WTQ(NDMHAR,NDMHAR), WTQS(NDMHAR,NDMHAR),
c###     &          WTPS(IPRAR,IPRAR)
c###      DIMENSION SSDT(ITMXP+1), SSSF(ITMXP+1)
c###      REAL OTIME(ND)
c###      INCLUDE 'param.inc'
c###C
c###      IF (IPAR.EQ.0 .OR. IPAR.EQ.-1) THEN
c###        IF (IERR.EQ.0) IERR = -1
c###        WRITE (IOUT,530) IP, KSTP, KSTP
c###  530   FORMAT (' *****WARNINGS ABOUT FOLLOWING TABLE : ',/,
c###     &          4X,'IP= ',I4,/,4X,
c###     &  'IF IP=0, DEPENDENT VARIABLES AT TIME STEPS AFTER'
c###     &  ,I5,' ARE CALCULATED WITH ',/,4X,'OLD PARAMETER VALUES.  ',
c###     &  'DEPENDENT VARIABLES CALCULATED AT TIME STEP',I5,/,4X,
c###     &  'ARE IN ERROR BECAUSE THE SOLVER DID NOT CONVERGE',
c###     &  ' (SEE BELOW)')
c###        CALL SOBS1BAS6OH(WP,IOUT,NH,H,HOBS,WT,OBSNAM,ND,MPR,PRM,RSQ,
c###     &                   RSQP,2,LN,IPR,NIPR,WTPS,D,R,WTQ,
c###     &                   WTQS,NDMH,NTT2,KTDIM,NPLIST,MPRAR,IPRAR,OUTNAM,
c###     &                   IPLOT,EQNAM,NAMES,IPLPTR,NDMHAR,NQTDR,NQTRV,
c###     &                   NQTGB,NQTST,NQTCH,IOWTQCH,IOWTQDR,IOWTQRV,
c###     &                   IOWTQGB,IOWTQST,LCOBBAS,LCOBDRN,LCOBRIV,
c###     &                   LCOBGHB,LCOBSTR,LCOBCHD,LCOBADV,-1,SSGF,SSDR,
c###     &                   SSRV,SSGB,SSST,SSAD,SSCH,SSPI,SSTO,ITMXP,IPES,
c###     &                   BPRI,LCOBDRT,SSDT,NQTDT,IOWTQDT,NRSO,NPOST,
c###     &                   NNEGT,NRUNS,NQTSF,IOWTQSF,LCOBSFR,SSSF,NHT,
c###     &                   OTIME)
c###      ENDIF
c###      IF (IP.EQ.0) THEN
c###        WRITE (IOUT,540) KSTP,KPER
c###        WRITE (IERRU,540) KSTP,KPER
c###        IF (IERR.EQ.0) IERR = -1
c###      ENDIF
c###  540 FORMAT (//,
c###     &' *****   SOLUTION DID NOT CONVERGE FOR HEADS   *****',/,
c###     &' ***** FOR TIME STEP ',I3,' IN STRESS PERIOD ',I4,' *****')
c###      IF (IP.GT.0) THEN
c###        WRITE (IOUT,545) PARNAM(IPPTR(IP)),PARTYP(IPPTR(IP)),
c###     &                              KSTP,KPER,ITERP
c###        WRITE (IERRU,545) PARNAM(IPPTR(IP)),PARTYP(IPPTR(IP)),
c###     &                              KSTP,KPER,ITERP
c###        IF (IERR.EQ.0) IERR = -1
c###      ENDIF
c###  545 FORMAT (//,1X,16('*'),'     WARNING     ',16('*'),/,
c###     &1X,'*',47X,'*'/,
c###     &' *  SOLUTION DID NOT CONVERGE FOR SENSITIVITIES  *',/,
c###     &1X,'*    FOR PARAMETER "',A10,'" OF TYPE "',A4,'"  *',/,
c###     &1X,'*    FOR TIME STEP ',I3,' IN STRESS PERIOD ',I4,'    *',/,
c###     &1X,'*    OF PARAMETER-ESTIMATION ITERATION ',I3,6X,'*',/,
c###     &1X,'*  IF THIS IS NOT THE FINAL ITERATION,          *',/,
c###     &1X,'*    THIS NONCONVERGENCE IS NOT A PROBLEM       *',/,
c###     &1X,'*',47X,'*'/,
c###     &1X,49('*'),/)
c###C
c###      RETURN
...
c###     1      180,190,200,210), IP
c###C
c###C------------FORMAT 11G10.3
c###   10 WRITE(IOUT,11) I,(BUF(J,I),J=J1,J2)
c###   11 FORMAT(1X,I3,2X,1PG10.3,10(1X,G10.3))
c###      GO TO 1000
c###C
c###C------------FORMAT 9G13.6
c###   20 WRITE(IOUT,21) I,(BUF(J,I),J=J1,J2)
c###   21 FORMAT(1X,I3,2X,1PG13.6,8(1X,G13.6))
c###      GO TO 1000
c###C
c###C------------FORMAT 15F7.1
c###   30 WRITE(IOUT,31) I,(BUF(J,I),J=J1,J2)
c###   31 FORMAT(1X,I3,1X,15(1X,F7.1))
c###      GO TO 1000
c###C
c###C------------FORMAT 15F7.2
c###   40 WRITE(IOUT,41) I,(BUF(J,I),J=J1,J2)
c###   41 FORMAT(1X,I3,1X,15(1X,F7.2))
c###      GO TO 1000
c###C
c###C------------FORMAT 15F7.3
c###   50 WRITE(IOUT,51) I,(BUF(J,I),J=J1,J2)
c###   51 FORMAT(1X,I3,1X,15(1X,F7.3))
c###      GO TO 1000
c###C
c###C------------FORMAT 15F7.4
c###   60 WRITE(IOUT,61) I,(BUF(J,I),J=J1,J2)
c###   61 FORMAT(1X,I3,1X,15(1X,F7.4))
c###      GO TO 1000
c###C
c###C------------FORMAT 20F5.0
c###   70 WRITE(IOUT,71) I,(BUF(J,I),J=J1,J2)
c###   71 FORMAT(1X,I3,1X,20(1X,F5.0))
c###      GO TO 1000
c###C
c###C------------FORMAT 20F5.1
c###   80 WRITE(IOUT,81) I,(BUF(J,I),J=J1,J2)
c###   81 FORMAT(1X,I3,1X,20(1X,F5.1))
c###      GO TO 1000
c###C
c###C------------FORMAT 20F5.2
c###   90 WRITE(IOUT,91) I,(BUF(J,I),J=J1,J2)
c###   91 FORMAT(1X,I3,1X,20(1X,F5.2))
c###      GO TO 1000
c###C
c###C------------FORMAT 20F5.3
c###  100 WRITE(IOUT,101) I,(BUF(J,I),J=J1,J2)
c###  101 FORMAT(1X,I3,1X,20(1X,F5.3))
c###      GO TO 1000
c###C
c###C------------FORMAT 20F5.4
c###  110 WRITE(IOUT,111) I,(BUF(J,I),J=J1,J2)
c###  111 FORMAT(1X,I3,1X,20(1X,F5.4))
c###      GO TO 1000
c###C
c###C------------FORMAT 10G11.4
c###  120 WRITE(IOUT,121) I,(BUF(J,I),J=J1,J2)
c###  121 FORMAT(1X,I3,2X,1PG11.4,9(1X,G11.4))
c###      GO TO 1000
c###C
c###C------------FORMAT 10F6.0
c###  130 WRITE(IOUT,131) I,(BUF(J,I),J=J1,J2)
c###  131 FORMAT(1X,I3,1X,10(1X,F6.0))
c###      GO TO 1000
c###C
c###C------------FORMAT 10F6.1
c###  140 WRITE(IOUT,141) I,(BUF(J,I),J=J1,J2)
c###  141 FORMAT(1X,I3,1X,10(1X,F6.1))
c###      GO TO 1000
c###C
c###C------------FORMAT 10F6.2
c###  150 WRITE(IOUT,151) I,(BUF(J,I),J=J1,J2)
c###  151 FORMAT(1X,I3,1X,10(1X,F6.2))
c###      GO TO 1000
c###C
c###C------------FORMAT 10F6.3
c###  160 WRITE(IOUT,161) I,(BUF(J,I),J=J1,J2)
c###  161 FORMAT(1X,I3,1X,10(1X,F6.3))
c###      GO TO 1000
c###C
c###C------------FORMAT 10F6.4
c###  170 WRITE(IOUT,171) I,(BUF(J,I),J=J1,J2)
c###  171 FORMAT(1X,I3,1X,10(1X,F6.4))
c###      GO TO 1000
c###C
c###C------------FORMAT 10F6.5
c###  180 WRITE(IOUT,181) I,(BUF(J,I),J=J1,J2)
c###  181 FORMAT(1X,I3,1X,10(1X,F6.5))
c###C
c###C------------FORMAT 5G12.5
c###  190 WRITE(IOUT,191) I,(BUF(J,I),J=J1,J2)
c###  191 FORMAT(1X,I3,1X,1PG12.5,4(1X,G12.5))
c###C
c###C------------FORMAT 6G11.4
c###  200 WRITE(IOUT,201) I,(BUF(J,I),J=J1,J2)
c###  201 FORMAT(1X,I3,1X,1PG11.4,5(1X,G11.4))
c###C
c###C------------FORMAT 7G9.2
c###  210 WRITE(IOUT,211) I,(BUF(J,I),J=J1,J2)
c###  211 FORMAT(1X,I3,1X,1PG9.2,6(1X,G9.2))
c###C
c### 1000 CONTINUE
 2000 CONTINUE
C
...
c###C
c###C------------ FORMAT 11G10.3
c###   10 WRITE(IOUT,11) I,(BUF(J,I),J=1,NCOL)
c###   11 FORMAT(1X,I3,2X,1PG10.3,10(1X,G10.3):/(5X,11(1X,G10.3)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 9G13.6
c###   20 WRITE(IOUT,21) I,(BUF(J,I),J=1,NCOL)
c###   21 FORMAT(1X,I3,2X,1PG13.6,8(1X,G13.6):/(5X,9(1X,G13.6)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 15F7.1
c###   30 WRITE(IOUT,31) I,(BUF(J,I),J=1,NCOL)
c###   31 FORMAT(1X,I3,1X,15(1X,F7.1):/(5X,15(1X,F7.1)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 15F7.2
c###   40 WRITE(IOUT,41) I,(BUF(J,I),J=1,NCOL)
c###   41 FORMAT(1X,I3,1X,15(1X,F7.2):/(5X,15(1X,F7.2)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 15F7.3
c###   50 WRITE(IOUT,51) I,(BUF(J,I),J=1,NCOL)
c###   51 FORMAT(1X,I3,1X,15(1X,F7.3):/(5X,15(1X,F7.3)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 15F7.4
c###   60 WRITE(IOUT,61) I,(BUF(J,I),J=1,NCOL)
c###   61 FORMAT(1X,I3,1X,15(1X,F7.4):/(5X,15(1X,F7.4)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 20F5.0
c###   70 WRITE(IOUT,71) I,(BUF(J,I),J=1,NCOL)
c###   71 FORMAT(1X,I3,1X,20(1X,F5.0):/(5X,20(1X,F5.0)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 20F5.1
c###   80 WRITE(IOUT,81) I,(BUF(J,I),J=1,NCOL)
c###   81 FORMAT(1X,I3,1X,20(1X,F5.1):/(5X,20(1X,F5.1)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 20F5.2
c###   90 WRITE(IOUT,91) I,(BUF(J,I),J=1,NCOL)
c###   91 FORMAT(1X,I3,1X,20(1X,F5.2):/(5X,20(1X,F5.2)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 20F5.3
c###  100 WRITE(IOUT,101) I,(BUF(J,I),J=1,NCOL)
c###  101 FORMAT(1X,I3,1X,20(1X,F5.3):/(5X,20(1X,F5.3)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 20F5.4
c###  110 WRITE(IOUT,111) I,(BUF(J,I),J=1,NCOL)
c###  111 FORMAT(1X,I3,1X,20(1X,F5.4):/(5X,20(1X,F5.4)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 10G11.4
c###  120 WRITE(IOUT,121) I,(BUF(J,I),J=1,NCOL)
c###  121 FORMAT(1X,I3,2X,1PG11.4,9(1X,G11.4):/(5X,10(1X,G11.4)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 10F6.0
c###  130 WRITE(IOUT,131) I,(BUF(J,I),J=1,NCOL)
c###  131 FORMAT(1X,I3,1X,10(1X,F6.0):/(5X,10(1X,F6.0)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 10F6.1
c###  140 WRITE(IOUT,141) I,(BUF(J,I),J=1,NCOL)
c###  141 FORMAT(1X,I3,1X,10(1X,F6.1):/(5X,10(1X,F6.1)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 10F6.2
c###  150 WRITE(IOUT,151) I,(BUF(J,I),J=1,NCOL)
c###  151 FORMAT(1X,I3,1X,10(1X,F6.2):/(5X,10(1X,F6.2)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 10F6.3
c###  160 WRITE(IOUT,161) I,(BUF(J,I),J=1,NCOL)
c###  161 FORMAT(1X,I3,1X,10(1X,F6.3):/(5X,10(1X,F6.3)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 10F6.4
c###  170 WRITE(IOUT,171) I,(BUF(J,I),J=1,NCOL)
c###  171 FORMAT(1X,I3,1X,10(1X,F6.4):/(5X,10(1X,F6.4)))
c###      GO TO 1000
c###C
c###C------------ FORMAT 10F6.5
c###  180 WRITE(IOUT,181) I,(BUF(J,I),J=1,NCOL)
c###  181 FORMAT(1X,I3,1X,10(1X,F6.5):/(5X,10(1X,F6.5)))
c###      GO TO 1000
c###C
c###C------------FORMAT 5G12.5
c###  190 WRITE(IOUT,191) I,(BUF(J,I),J=1,NCOL)
c###  191 FORMAT(1X,I3,2X,1PG12.5,4(1X,G12.5):/(5X,5(1X,G12.5)))
c###      GO TO 1000
c###C
c###C------------FORMAT 6G11.4
c###  200 WRITE(IOUT,201) I,(BUF(J,I),J=1,NCOL)
c###  201 FORMAT(1X,I3,2X,1PG11.4,5(1X,G11.4):/(5X,6(1X,G11.4)))
c###      GO TO 1000
c###C
c###C------------FORMAT 7G9.2
c###  210 WRITE(IOUT,211) I,(BUF(J,I),J=1,NCOL)
c###  211 FORMAT(1X,I3,2X,1PG9.2,6(1X,G9.2):/(5X,7(1X,G9.2)))
c###C
c### 1000 CONTINUE
C
C5------RETURN
...
C4B-----LOCAT>0; READ FORMATTED RECORDS USING FORMAT FMTIN.
   90 CONTINUE
c###      WRITE(IOUT,5) ANAME,LOCAT,FMTIN
    5 FORMAT(1X,///11X,A,/
...
c###     &                 ACTION=ACTION(1))
                  ...
                  OPEN(UNIT=LOCAT,FILE=FNAME,FORM=fmtarg,
     &                 ACTION='READ')
               ELSE
                  OPEN(UNIT=LOCAT,FILE=FNAME,ACTION='READ')
               END IF
...
C4A-----LOCAT=0; SET ALL ARRAY VALUES EQUAL TO ICONST. RETURN.
   50 DO 80 I=1,II
      DO 80 J=1,JJ
   80 IA(J,I)=ICONST
c###      IF(K.GT.0) WRITE(IOUT,82) ANAME,ICONST,K
   82 FORMAT(1X,/1X,A,' =',I15,' FOR LAYER',I4)
c###      IF(K.LE.0) WRITE(IOUT,83) ANAME,ICONST
...
500   continue
c###      DO 510 I=1,II
c###      GO TO(501,502,503,504,505,506,507,508,509), IPRN
c###C
c###C----------------FORMAT 60I1
c###  501 WRITE(IOUT,551) I,(IA(J,I),J=1,JJ)
c###  551 FORMAT(1X,I3,1X,60(1X,I1):/(5X,60(1X,I1)))
c###      GO TO 510
c###C
c###C----------------FORMAT 40I2
c###  502 WRITE(IOUT,552) I,(IA(J,I),J=1,JJ)
c###  552 FORMAT(1X,I3,1X,40(1X,I2):/(5X,40(1X,I2)))
c###      GO TO 510
c###C
c###C----------------FORMAT 30I3
c###  503 WRITE(IOUT,553) I,(IA(J,I),J=1,JJ)
c###  553 FORMAT(1X,I3,1X,30(1X,I3):/(5X,30(1X,I3)))
c###      GO TO 510
c###C
c###C----------------FORMAT 25I4
c###  504 WRITE(IOUT,554) I,(IA(J,I),J=1,JJ)
c###  554 FORMAT(1X,I3,1X,25(1X,I4):/(5X,25(1X,I4)))
c###      GO TO 510
c###C
c###C----------------FORMAT 20I5
c###  505 WRITE(IOUT,555) I,(IA(J,I),J=1,JJ)
c###  555 FORMAT(1X,I3,1X,20(1X,I5):/(5X,20(1X,I5)))
c###      GO TO 510
c###C
c###C----------------FORMAT 10I11
c###  506 WRITE(IOUT,556) I,(IA(J,I),J=1,JJ)
c###  556 FORMAT(1X,I3,1X,10(1X,I11):/(5X,10(1X,I11)))
c###      GO TO 510
c###C
c###C----------------FORMAT 25I2
c###  507 WRITE(IOUT,557) I,(IA(J,I),J=1,JJ)
c###  557 FORMAT(1X,I3,1X,25(1X,I2):/(5X,25(1X,I2)))
c###      GO TO 510
c###C
c###C----------------FORMAT 15I4
c###  508 WRITE(IOUT,558) I,(IA(J,I),J=1,JJ)
c###  558 FORMAT(1X,I3,1X,15(1X,I4):/(5X,10(1X,I4)))
c###      GO TO 510
c###C
c###C----------------FORMAT 10I6
c###  509 WRITE(IOUT,559) I,(IA(J,I),J=1,JJ)
c###  559 FORMAT(1X,I3,1X,10(1X,I6):/(5X,10(1X,I6)))
c###C
c###  510 CONTINUE
C
C9------RETURN
...
c###      IF(IOUT.GT.0) WRITE(IOUT,1) TEXT,IBDCHN,KSTP,KPER
    1 FORMAT(1X,'UBDSV1 SAVING "',A16,'" ON UNIT',I4,
     1     ' AT TIME STEP',I3,', STRESS PERIOD',I4)
c###      WRITE(IBDCHN) KSTP,KPER,TEXT,NCOL,NROW,-NLAY
c###      WRITE(IBDCHN) 1,DELT,PERTIM,TOTIM
C
C2------WRITE AN UNFORMATTED RECORD CONTAINING VALUES FOR
C2------EACH CELL IN THE GRID.
c###      WRITE(IBDCHN) BUFF
...
c###      IF(IOUT.GT.0) WRITE(IOUT,1) TEXT,IBDCHN,KSTP,KPER
    1 FORMAT(1X,'UBDSV2 SAVING "',A16,'" ON UNIT',I4,
     1     ' AT TIME STEP',I3,', STRESS PERIOD',I4)
c###      WRITE(IBDCHN) KSTP,KPER,TEXT,NCOL,NROW,-NLAY
c###      WRITE(IBDCHN) 2,DELT,PERTIM,TOTIM
c###      WRITE(IBDCHN) NLIST
C
C2------RETURN
      RETURN
      END
...
c###      IF(IOUT.GT.0) WRITE(IOUT,1) TEXT,IBDCHN,KSTP,KPER
    1 FORMAT(1X,'UBDSV4 SAVING "',A16,'" ON UNIT',I4,
     1     ' AT TIME STEP',I3,', STRESS PERIOD',I4)
c###      WRITE(IBDCHN) KSTP,KPER,TEXT,NCOL,NROW,-NLAY
c###      WRITE(IBDCHN) 5,DELT,PERTIM,TOTIM
c###      WRITE(IBDCHN) NAUX+1
c###      IF(NAUX.GT.0) WRITE(IBDCHN) (AUXTXT(N),N=1,NAUX)
c###      WRITE(IBDCHN) NLIST
C
C2------RETURN
      RETURN
...
C2------WRITE CELL NUMBER AND FLOW RATE
      IF(NAUX.GT.0) THEN
         N2=LAUX+NAUX-1
c###         WRITE(IBDCHN) ICRL,Q,(VAL(N),N=LAUX,N2)
      ELSE
c###         WRITE(IBDCHN) ICRL,Q
      END IF
C
...
c###      IF(LBLSAV.NE.0) WRITE(ICHN,5) KSTP,KPER,PERTIM,TOTIM,TEXT,NCOL,
c###     1                 NROW,ILAY,FMTOUT
5     FORMAT(1X,2I5,1P,2E15.6,1X,A,3I6,1X,A)
C
C2------WRITE THE ARRAY USING THE SPECIFIED FORMAT.
      DO 10 IR=1,NROW
c###      WRITE(ICHN,FMTOUT) (BUFF(IC,IR),IC=1,NCOL)
10    CONTINUE
C
C3------RETURN
      RETURN
...
C2------WRITE THE ARRAY USING THE SPECIFIED FORMAT.
      DO 10 IR=1,NROW
c###      WRITE(ICHN,FMTOUT) (IDATA(IC,IR),IC=1,NCOL)
10    CONTINUE
C
C3------RETURN.
      RETURN
...
C     ------------------------------------------------------------------
   ...
   20 CONTINUE
   30 continue
c###   IF (IOUT.GT.0) WRITE(IOUT,'(1X,A)') LINE(1:I)
      GO TO 10
C
      END
...
C  Check for and decode EXTERNAL and SFAC records.
      ...
      IF(LINE(ISTART:ISTOP).EQ.'EXTERNAL') THEN
         ...
         IN=I
c###         IF(ITERP.EQ.1)WRITE(IOUT,111) IN
  111    FORMAT(1X,'Reading list on unit ',I4)
      ...
      ELSE IF(LINE(ISTART:ISTOP).EQ.'OPEN/CLOSE') THEN
         ...
         IN=NUNOPN
c###         IF(ITERP.EQ.1)WRITE(IOUT,115) IN,FNAME
  115    FORMAT(1X,/1X,'OPENING FILE ON UNIT ',I4,':',/1X,A)
         OPEN(UNIT=IN,FILE=FNAME,ACTION='READ')
         ICLOSE=1
         READ(IN,'(A)') LINE
      END IF
      LLOC=1
...
C  Write the values that were read.
      NN=II-LSTBEG+1
c###      IF(ITERP.EQ.1) WRITE(IOUT,205) NN,K,I,J,(RLIST(JJ,II),JJ=4,NREAD2)
...
C
C  Write the label.
c###      WRITE(IOUT,103) BUF(1:NBUF)
c###      WRITE(IOUT,104) (DASH(J),J=1,NBUF)
  103 FORMAT(1X,A)
  104 FORMAT(1X,160A)
C
      RETURN
...
C     ------------------------------------------------------------------
c###      WRITE(IOUT,*)
c###      WRITE(IOUT,'(1X,2A)') TEXT1,TEXT2
C
      RETURN
      END
...
c###        WRITE(IOUT,200) NDIG,MINCOLSP
...
c###C------------ FORMAT 11G10.3
c###        IF (IP.EQ.1) THEN
c###          WRITE(IOUT,11) PARNAM(IPPTR(I)),(BUF(J,I),J=1,NPE)
c###   11     FORMAT(1X,A,1X,1PG10.3,10(1X,G10.3):/(11X,11(1X,G10.3)))
c###C
c###C------------ FORMAT 10G11.4
c###        ELSEIF (IP.EQ.2) THEN
c###          WRITE(IOUT,21) PARNAM(IPPTR(I)),(BUF(J,I),J=1,NPE)
c###   21     FORMAT(1X,A,1X,1PG11.4,9(1X,G11.4):/(11X,10(1X,G11.4)))
c###C
c###C------------ FORMAT 9G12.5
c###        ELSEIF (IP.EQ.3) THEN
c###          WRITE(IOUT,31) PARNAM(IPPTR(I)),(BUF(J,I),J=1,NPE)
c###   31     FORMAT(1X,A,1X,1PG12.5,8(1X,G12.5):/(11X,9(1X,G12.5)))
c###C
c###C------------ FORMAT 8G13.6
c###        ELSEIF (IP.EQ.4) THEN
c###          WRITE(IOUT,41) PARNAM(IPPTR(I)),(BUF(J,I),J=1,NPE)
c###   41     FORMAT(1X,A,1X,1PG13.6,7(1X,G13.6):/(11X,8(1X,G13.6)))
c###C
c###C------------ FORMAT 8G14.7
c###        ELSEIF (IP.EQ.5) THEN
c###          WRITE(IOUT,51) PARNAM(IPPTR(I)),(BUF(J,I),J=1,NPE)
c###   51     FORMAT(1X,A,1X,1PG14.7,7(1X,G14.7):/(11X,8(1X,G14.7)))
c###C
c###C------------ FORMAT 6G10.3
c###        ELSEIF (IP.EQ.6) THEN
c###          WRITE(IOUT,61) PARNAM(IPPTR(I)),(BUF(J,I),J=1,NPE)
c###   61     FORMAT(1X,A,1X,1PG10.3,5(1X,G10.3):/(11X,6(1X,G10.3)))
c###C
c###C------------ FORMAT 5G11.4
c###        ELSEIF (IP.EQ.7) THEN
c###          WRITE(IOUT,71) PARNAM(IPPTR(I)),(BUF(J,I),J=1,NPE)
c###   71     FORMAT(1X,A,1X,1PG11.4,4(1X,G11.4):/(11X,5(1X,G11.4)))
c###C
c###C------------ FORMAT 5G12.5
c###        ELSEIF (IP.EQ.8) THEN
c###          WRITE(IOUT,81) PARNAM(IPPTR(I)),(BUF(J,I),J=1,NPE)
c###   81     FORMAT(1X,A,1X,1PG12.5,4(1X,G12.5):/(11X,5(1X,G12.5)))
c###C
c###C------------ FORMAT 4G13.6
c###        ELSEIF (IP.EQ.9) THEN
c###          WRITE(IOUT,91) PARNAM(IPPTR(I)),(BUF(J,I),J=1,NPE)
c###   91     FORMAT(1X,A,1X,1PG13.6,3(1X,G13.6):/(11X,4(1X,G13.6)))
c###C
c###C------------ FORMAT 4G14.7
c###        ELSEIF (IP.EQ.10) THEN
c###          WRITE(IOUT,101) PARNAM(IPPTR(I)),(BUF(J,I),J=1,NPE)
c###  101     FORMAT(1X,A,1X,1PG14.7,3(1X,G14.7):/(11X,4(1X,G14.7)))
c###C
c###        ENDIF
c### 1000 CONTINUE
C
...
c###C------------ FORMAT 10G12.3
c###        IF (IP.EQ.1) THEN
c###          WRITE(IOUT,11) OBSNAM(I),(BUF(J,I),J=IC1,IC2)
c###   11     FORMAT(1X,A,1X,1PG12.3,9(1X,G12.3):/(11X,10(1X,G12.3)))
c###C
c###C------------ FORMAT 10G12.4
c###        ELSEIF (IP.EQ.2) THEN
c###          WRITE(IOUT,21) OBSNAM(I),(BUF(J,I),J=IC1,IC2)
c###   21     FORMAT(1X,A,1X,1PG12.4,9(1X,G12.4):/(11X,10(1X,G12.4)))
c###C
c###C------------ FORMAT 9G12.5
c###        ELSEIF (IP.EQ.3) THEN
c###          WRITE(IOUT,31) OBSNAM(I),(BUF(J,I),J=IC1,IC2)
c###   31     FORMAT(1X,A,1X,1PG12.5,8(1X,G12.5):/(11X,9(1X,G12.5)))
c###C
c###C------------ FORMAT 8G13.6
c###        ELSEIF (IP.EQ.4) THEN
c###          WRITE(IOUT,41) OBSNAM(I),(BUF(J,I),J=IC1,IC2)
c###   41     FORMAT(1X,A,1X,1PG13.6,7(1X,G13.6):/(11X,8(1X,G13.6)))
c###C
c###C------------ FORMAT 8G14.7
c###        ELSEIF (IP.EQ.5) THEN
c###          WRITE(IOUT,51) OBSNAM(I),(BUF(J,I),J=IC1,IC2)
c###   51     FORMAT(1X,A,1X,1PG14.7,7(1X,G14.7):/(11X,8(1X,G14.7)))
c###C
c###C------------ FORMAT 5G12.3
c###        ELSEIF (IP.EQ.6) THEN
c###          WRITE(IOUT,61) OBSNAM(I),(BUF(J,I),J=IC1,IC2)
c###   61     FORMAT(1X,A,1X,1PG12.3,4(1X,G12.3):/(11X,5(1X,G12.3)))
c###C
c###C------------ FORMAT 5G12.4
c###        ELSEIF (IP.EQ.7) THEN
c###          WRITE(IOUT,71) OBSNAM(I),(BUF(J,I),J=IC1,IC2)
c###   71     FORMAT(1X,A,1X,1PG12.4,4(1X,G12.4):/(11X,5(1X,G12.4)))
c###C
c###C------------ FORMAT 5G12.5
c###        ELSEIF (IP.EQ.8) THEN
c###          WRITE(IOUT,81) OBSNAM(I),(BUF(J,I),J=IC1,IC2)
c###   81     FORMAT(1X,A,1X,1PG12.5,4(1X,G12.5):/(11X,5(1X,G12.5)))
c###C
c###C------------ FORMAT 4G13.6
c###        ELSEIF (IP.EQ.9) THEN
c###          WRITE(IOUT,91) OBSNAM(I),(BUF(J,I),J=IC1,IC2)
c###   91     FORMAT(1X,A,1X,1PG13.6,3(1X,G13.6):/(11X,4(1X,G13.6)))
c###C
c###C------------ FORMAT 4G14.7
c###        ELSEIF (IP.EQ.10) THEN
c###          WRITE(IOUT,101) OBSNAM(I),(BUF(J,I),J=IC1,IC2)
c###  101     FORMAT(1X,A,1X,1PG14.7,3(1X,G14.7):/(11X,4(1X,G14.7)))
c###C
c###        ENDIF
c### 1000 CONTINUE
