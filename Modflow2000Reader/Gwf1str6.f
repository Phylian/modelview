     ...
     2              LCTBAR,LCTRIB,LCIVAR,LCFGAR,NPSTR,ISTRPB,ierror)
C
...
C2------ READ MXACTS, NSS, NTRIB, ISTCB1, AND ISTCB2.
    ...
    3 FORMAT(5I10,F10.0,2I10)
      ...
      IF(NSS.LT.0)NSS=0
C###      WRITE(IOUT,4)MXACTS,NSS,NTRIB
...
C10-----RETURN.
     ...
     1         ISTRPB,MXSTRM,ITERP,INAMLOC,ierror)
C
...
C         READ LIST(S) OF CELLS, PRECEDED BY INSTANCE NAME IF NUMINST>0
     ...
     &                      IOUT,NCOL,NROW,NLAY,IRDFLG, ierror)
...
C###               STOP
   ...
   40       CONTINUE
            ...
            IF(IRDFLG.EQ.0) THEN
C###               IF (N.EQ.1 .AND. I.EQ.1) WRITE(IOUT,4)
...
C###               WRITE(IOUT,6)(ISTRM(J,II),J=1,5),(STRM(J,II),J=1,5)
            ENDIF
...
C###      IF(IRDFLG.EQ.0) WRITE(IOUT,7)
    ...
    8 FORMAT(3F10.0)
C###      IF(IRDFLG.EQ.0) WRITE(IOUT,9)ISTRM(1,II),
C###     &    ISTRM(2,II),ISTRM(3,II),ISTRM(4,II),ISTRM(5,II),
C###     1    STRM(6,II),STRM(7,II),STRM(8,II)
...
C###      WRITE(IOUT,650)
  650 FORMAT(/,12X,'LAYER',6X,'ROW',5X,'COLUMN',5X,'STREAM',4X,
     ...
     3      6X,'STREAM REACH'//)
      ...
      IR=ISTRM(2,L)
      IC=ISTRM(3,L)
C###      WRITE(IOUT,675)IL,IR,IC,ISTRM(4,L),ISTRM(5,L),
C###     1     STRM(10,L),STRM(11,L),STRM(9,L)
...
C###      WRITE(IOUT,710)
  710 FORMAT(/,12X,'LAYER',6X,'ROW',5X,'COLUMN',5X,'STREAM',4X,
     ...
     3 4X,'AQUIFER',6X,'STREAM REACH',5X,'STREAM'//)
      ...
      IR=ISTRM(2,L)
      IC=ISTRM(3,L)
C###      WRITE(IOUT,775)IL,IR,IC,ISTRM(4,L),ISTRM(5,L),
C###     1     STRM(10,L),STRM(11,L),STRM(9,L),STRM(2,L)
...
C31-----RETURN.                                                        C
     ...
     1          IOUT,NCOL,NROW,NLAY,IRDFLG,ierror)
...
C###      IF(IRDFLG.EQ.0) WRITE(IOUT,6)K,I,J,ISTRM(4,II),
C###     1ISTRM(5,II),STRM(1,II),STRM(2,II),STRM(3,II),STRM(4,II),STRM(5,II)
    6 FORMAT(1X,3X,I4,2I7,2I9,7X,G11.4,G12.4,G11.4,4X,2G13.4)
      ISTRM(1,II)=K
      ISTRM(2,II)=I
      ISTRM(3,II)=J
...
C###         WRITE(IOUT,*) ' Layer number in list is outside of the grid'
	          ierror=20
	          return
C###         STOP
      END IF
      IF(I.LT.1 .OR. I.GT.NROW) THEN
C###         WRITE(IOUT,*) ' Row number in list is outside of the grid'
	          ierror=20
	          return
C###         STOP
      END IF
      IF(J.LT.1 .OR. J.GT.NCOL) THEN
C###         WRITE(IOUT,*) ' Column number in list is outside of the grid'
	          ierror=20
	          return
C###         STOP
      END IF
