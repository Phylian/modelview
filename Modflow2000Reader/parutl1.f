...
C###         WRITE(IOUT,31) NP
   31    FORMAT(1X,I5,' Named Parameters     ')
      ELSE
         NP=0
C###         WRITE(IOUT,'(A)') ' No named parameters'
      END IF
C
      RETURN
      END
...
C###              WRITE(IOUT,12) PARNAM(NP),PTYP
...
C###          WRITE(IOUT,*) NP,NCLU
C###          WRITE(IOUT,'(A)') PARNAM(NP)
C###          WRITE(IOUT,'(4I10)') IPLOC
          CALL USTOP(' ')
        END IF
C###        WRITE(IOUT,121) PARNAM(NP),PARTYP(NP),NCLU
...
C  specified layer.
     ...
     1           NZONAR,NSUB)
         INIT=0
C###         IF(NSUB.GT.0) WRITE(IOUT,47) PARNAM(IP)
   47    FORMAT(1X,A)
      END IF
  100 CONTINUE
C
...
C###        WRITE(IOUT,5) LINE(ISTART:ISTOP)
    5   FORMAT(' Parameter:  ',A)
        IF(LINE(ISTART:ISTOP).EQ.' ') THEN
C###          WRITE(IOUT,*) ' Blank parameter name in the ',PACK,' file.'
          CALL USTOP(' ')
        END IF
C
...
C###          WRITE(IOUT,83) PARNAM(IP),PARTYP(IP),PACK,PTYP
...
C     Check to see if entire array is a constant.
  ...
  290   CONTINUE
  300 CONTINUE
C###      IF(ILAY.GT.0) WRITE(IOUT,302) ANAME,TMP,ILAY
  302 FORMAT(1X,A,' =',1P,G14.6,' FOR LAYER',I4)
C###      IF(ILAY.LE.0) WRITE(IOUT,303) ANAME,TMP
  303 FORMAT(1X,A,' =',1P,G14.6)
      RETURN
C
C     Print the array.
  400 IF(ILAY.GT.0) THEN
C###        WRITE(IOUT,494) ANAME,ILAY
  494   FORMAT(1X,//11X,A,' FOR LAYER',I4)
      ELSE IF(ILAY.EQ.0) THEN
C###        WRITE(IOUT,495) ANAME
  495   FORMAT(1X,//11X,A)
      ELSE
C###        WRITE(IOUT,496) ANAME
...
C###        WRITE(IOUT,121) PARNAM(NP),PARTYP(NP)
...
C###          WRITE(IOUT,134) LSTSUM-1,MXLST
  134     FORMAT(1X,'EXCEEDED THE MAXIMUM NUMBER OF LIST ENTRIES:'/
     ...
     2          1X,'The maximum number of list entries is',I5)
        ...
        END IF
        IF((INAMLOC-1).GT.MXINST) THEN
C###          WRITE(IOUT,135)INAMLOC-1,MXINST
...
C     READ INSTANCE NAME
      ...
      CTMP1 = LINE(ISTART:ISTOP)
      CALL UPCASE(CTMP1)
C###      IF (ITERP.EQ.1) WRITE(IOUT,1010)INAME(ILOC)
      IF (I.GT.1) THEN
        DO 10 J=IPL4,IPL4+I-2
          CTMP2 = INAME(J)
          CALL UPCASE(CTMP2)
          IF (CTMP1.EQ.CTMP2) THEN
C###            WRITE(IOUT,1020)INAME(J)
            CALL USTOP(' ')
          ENDIF
   10   CONTINUE
      ENDIF
...
C
      ...
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,0,IDUM,RDUM,IOUT,IN)
C###      WRITE(IOUT,1) LINE(ISTART:ISTOP)
...
C
      ...
      DO 100 IP=1,MXPAR
        ...
        IF(CTMP1.EQ.CTMP2) THEN
          IF(PARTYP(IP).NE.PTYP) THEN
C###            WRITE(IOUT,81) PARNAM(IP),PARTYP(IP),PACK,PTYP
...
C  Substitute values
   ...
   86       CONTINUE
            ...
            IR=RLIST(2,II)
            IC=RLIST(3,II)
C###            IF (IOUTU.GT.0) WRITE(IOUT,89) II,IL,IR,IC,
C###     &          (RLIST(JJ,II),JJ=4,NREAD)
  ...
  100 CONTINUE
C
C###      WRITE(IOUT,*) ' The ',PACK,
C###     1   ' file specifies an undefined parameter:',LINE(ISTART:ISTOP)
      CALL USTOP(' ')
C
C
      END
...
C
      ...
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,0,IDUM,RDUM,IOUT,IN)
C###      WRITE(IOUT,500) LINE(ISTART:ISTOP)
      IF(LINE(ISTART:ISTOP).EQ.' ') THEN
C###        WRITE(IOUT,*) ' Blank parameter name in the ',PACK,' file.'
        CALL USTOP(' ')
      END IF
C
      ...
      DO 100 IP=1,MXPAR
        ...
        IF(CTMP1.EQ.CTMP2) THEN
          IF(PARTYP(IP).NE.PTYP) THEN
C###            WRITE(IOUT,510) PARNAM(IP),PARTYP(IP),PACK,PTYP
            CALL USTOP(' ')
          END IF
C
C         DESIGNATE CELLS CORRESPONDING TO CORRECT PARAMETER INSTANCE
          ...
          IF(NUMINST.GT.0) THEN
            ...
            CTMP3=LINE(ISTART:ISTOP)
            IF(CTMP3.EQ.' ') THEN
C###              WRITE(IOUT,520)PACK,PARNAM(IP)
              CALL USTOP(' ')
            ENDIF
C###            WRITE(IOUT,530) CTMP3
            CALL UPCASE(CTMP3)
            DO 50 KI=1,NUMINST
              CTMP4=INAME(ILOC+KI-1)
              CALL UPCASE(CTMP4)
              ...
              ENDIF
   50       CONTINUE
C###            WRITE(IOUT,540) PACK,CTMP3,PARNAM(IP)
            CALL USTOP(' ')
   60       CONTINUE
          ENDIF
C
          IF (IACTIVE(IP).GT.0) THEN
C###            WRITE(IOUT,550) PARNAM(IP)
            CALL USTOP(' ')
          ENDIF
C
          IACTIVE(IP)=NI
...
C
C###      WRITE(IOUT,*) ' The ',PACK,
C###     &   ' file specifies an undefined parameter:',LINE(ISTART:ISTOP)
      CALL USTOP(' ')
C
      END
