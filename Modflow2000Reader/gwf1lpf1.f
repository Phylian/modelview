...
c###      WRITE(IOUT,47)
   47 FORMAT(1X,/3X,'LAYER FLAGS:',/1X,
     ...
     2 '       LAYVKA        LAYWET',/1X,75('-'))
      DO 50 K=1,NLAY
c###      WRITE(IOUT,48) K,LAYTYP(K),LAYAVG(K),CHANI(K),LAYVKA(K),LAYWET(K)
...
c###            WRITE(IOUT,*)
c###     1          ' LAYWET is not 0 and LAYTYP is 0 for layer:',K
c###            WRITE(IOUT,*) ' LAYWET must be 0 if LAYTYP is 0'
c###            CALL USTOP(' ')
            ierror=20
            return
         ELSE
...
C###                CALL USTOP(' ')
   ...
   17       CONTINUE
         ...
         ELSE IF(PTYP.EQ.'SY') THEN
            NPSY=1
         ELSE
C###            WRITE(IOUT,*) ' Invalid parameter type for LPF Package'
C###            CALL USTOP(' ')
            ierror=20
            return
         END IF
...
C###         IF (ISEN.NE.0 .AND. (PTYP.EQ.'HK' .OR. PTYP.EQ.'HANI')) THEN
C###           IF (ISENS(N).GT.0) THEN
C            CYCLE THROUGH CLUSTERS INCLUDED IN THIS PARAMETER
C###             DO 19 ICLUSTER=IPLOC(1,N),IPLOC(2,N)
C###               LAYER=IPCLST(1,ICLUSTER)
C###               LAVG=LAYAVG(LAYER)
C###               IF (LAVG.NE.0) THEN
C###                 WRITE(IOUT,18)PARNAM(N),LAYER,AVGNAM(LAVG)
C###   18            FORMAT(/,1X,'*** ERROR: Sensitivity for parameter ',A,
C###     &               ' cannot be calculated',/,1X,'because interblock ',
C###     &               'transmissivity averaging method ',
C###     &               'for layer ',I3,' is',/,1X,'specified as ',A,
C###     &               '.  To calculate sensitivity for "HK" or "HANI" ',
C###     &               /,1X,'type parameters, specify averaging ',
C###     &               'method as HARMONIC MEAN.',/,1X,
C###     &               '-- STOP EXECUTION (GWF1LPF1RPGD)')
C###                 CALL USTOP(' ')
C###               ENDIF
C###   19        CONTINUE
C###           END IF
...
C2B-----DEFINE HORIZONTAL HYDRAULIC CONDUCTIVITY (HK)
      ...
      ELSE
         READ(IN,*) LAYFLG(1,K)
C###         WRITE(IOUT,101) ANAME(1),K,LAYFLG(1,K)
...
C%ERA%HANI
        ...
        ELSE
           READ(IN,*) LAYFLG(6,K)
C###           WRITE(IOUT,101) ANAME(2),K,LAYFLG(6,K)
        END IF
      END IF
C
...
C2D-----ANISOTROPY (VKA).
      ...
      ELSE
         READ(IN,*) LAYFLG(2,K)
C###         WRITE(IOUT,101) ANAME(IANAME),K,LAYFLG(2,K)
      END IF
C
C2E-----DEFINE SPECIFIC STORAGE IN ARRAY SC1 IF TRANSIENT.
     ...
     1            DELR,DELC,1)
         ELSE
            READ(IN,*) LAYFLG(3,K)
C###            WRITE(IOUT,101) ANAME(6),K,LAYFLG(3,K)
         END IF
      END IF
C
...
C2F-----IS CONVERTIBLE.
     ...
     1                 NBOTM,DELR,DELC,0)
            ELSE
               READ(IN,*) LAYFLG(4,K)
C###               WRITE(IOUT,101) ANAME(7),K,LAYFLG(4,K)
            END IF
         END IF
      END IF
C
C2G-----READ CONFINING BED VERTICAL HYDRAULIC CONDUCTIVITY (VKCB)
     ...
     1             IOUT)
         ELSE
            READ(IN,*) LAYFLG(5,K)
C###            WRITE(IOUT,101) ANAME(5),K,LAYFLG(5,K)
         END IF
      END IF
C
...
C4------RETURN
     ...
     2 NBOTM,RMLT,IZON,NMLTAR,NZONAR,LAYFLG,BUFF,ITERPK,ierror)
C
...
C
C2E-----DEFINE SPECIFIC STORAGE IN ARRAY SC1 IF TRANSIENT.
c###      IF(ITRSS.NE.0) THEN
c###         IF(NPSS.NE.0) THEN
c###            CALL UPARARRSUB1(SC1(1,1,KK),NCOL,NROW,KK,'SS',
c###     1      IOUT,ANAME(6),LAYFLG(3,KK),RMLT,IZON,NMLTAR,NZONAR)
c###            CALL SGWF1LPF1SC(SC1(1,1,KK),NCOL,NROW,KK,BOTM,NBOTM,
c###     1            DELR,DELC,1)
c###            IF (ITERPK.EQ.1)
c###     &          CALL UPARARRCK(BUFF,IBOUND,IOUT,IZON,K,NCOL,NLAY,NROW,
c###     &                         NZONAR,'SS  ')
c###         END IF
c###      END IF
C
C2F-----DEFINE SPECIFIC YIELD IN ARRAY SC2 IF TRANSIENT AND LAYER IS
C2F-----IS CONVERTIBLE.
c###      IF(LAYTYP(K).NE.0) THEN
c###         IF(ITRSS.NE.0) THEN
c###            IF(NPSY.NE.0) THEN
c###               CALL UPARARRSUB1(SC2(1,1,LAYTYP(K)),NCOL,
c###     1         NROW,KK,'SY',IOUT,ANAME(7),LAYFLG(4,KK),RMLT,IZON,
c###     2         NMLTAR,NZONAR)
c###               CALL SGWF1LPF1SC(SC2(1,1,LAYTYP(K)),NCOL,NROW,KK,BOTM,
c###     1             NBOTM,DELR,DELC,0)
c###               IF (ITERPK.EQ.1)
c###     &             CALL UPARARRCK(BUFF,IBOUND,IOUT,IZON,K,NCOL,NLAY,
c###     &                            NROW,NZONAR,'SY  ')
c###            END IF
c###         END IF
c###      END IF
