C     Last change:  ERB  11 Apr 2000    4:11 pm
...
C6------RETURN
     ...
     1      IPCBEG,MXCHD,IFREFM,ITERP,INAMLOC)
C
...
C###      IF (ITERP.EQ.1) WRITE(IOUT,3) NPCHD
...
C3------RETURN.
     ...
     1          IOUT,NCHDVL,IFREFM,NNPCHD,NPCHD,IPCBEG,ierror)
C
C
...
C5------SET IBOUND NEGATIVE AT SPECIFIED-HEAD CELLS.
      ...
      IR=CHDS(2,II)
      IC=CHDS(3,II)
c      IF(IBOUND(IC,IR,IL).GT.0) IBOUND(IC,IR,IL)=-IBOUND(IC,IR,IL)
      IF(IBOUND(IC,IR,IL).EQ.0) THEN
C###         WRITE(IOUT,6) IL,IR,IC
