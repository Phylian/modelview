C     Last change:  ERB  29 Aug 2002    1:22 pm
     ...
     4                    IBDT,SHOWPROG,NOTICECOUNT,mxunit,ierror)
C
...
c###    2 FORMAT(1X,'Run start date and time (yyyy/mm/dd hh:mm:ss): ',
c###     &I4,'/',I2.2,'/',I2.2,1X,I2,':',I2.2,':',I2.2,/)
C
...
C###         WRITE(IOUTG,6)
    6    FORMAT (1X,'THE FREE FORMAT OPTION HAS BEEN SELECTED')
      ELSEIF(LINE(ISTART:ISTOP).EQ.'PRINTTIME') THEN
         IPRTIM=1
C###         WRITE(IOUTG,2) (IBDT(I),I=1,3),(IBDT(I),I=5,7)
...
C=======================================================================
     ...
     4          NZONAR,NML,NZN,LCRMLT,LCIZON,IGWT,ierror)
C
...
C13-----READ PARAMETER VALUES FILE.
      CALL SGWF2BAS7ARPVAL(IUPVAL,IOUT,IUNIT,NIUNIT,ierror)
...
C4------CHECK FOR A VALID FILE TYPE.
      ...
      FILACT=' '
	skip=.false.
c*****Note that the OPEN statement below ignores ACCARG, FILSTAT, and FILACT.
c*****All files will be opened with ACCESS=SEQUENTIAL, FILSTAT='OLD', and
c*****FILACT='READ'.
C
...
c###      IF (FILSTAT.EQ.'UNKNOWN') THEN
c###        CALL URWORD(LINE,LLOC,IOPT1,IOPT2,1,N,R,IOUT,INUNIT)
c###        IF (LINE(IOPT1:IOPT2).EQ.'REPLACE' .OR.
c###     &      LINE(IOPT1:IOPT2).EQ.'OLD')
c###     &      FILSTAT = LINE(IOPT1:IOPT2)
c###      ENDIF
c###      IF (FILACT.EQ.' ') FILACT=ACTION(2)
      IF(.NOT.MASTER) THEN
        IF(LINE(ITYP1:ITYP2).EQ.'GLOBAL') THEN
          IFLEN=12
...
c###     1     LINE(ITYP1:ITYP2),IU,FILSTAT,FMTARG,ACCARG
...
c###      WRITE(*,2010)FNAME(1:IFLEN),IU,FILSTAT,FMTARG,ACCARG,FILACT
c###      WRITE(IOUTG,2010)FNAME(1:IFLEN),IU,FILSTAT,FMTARG,ACCARG,FILACT
c### 2010 FORMAT(/,1X,'*** ERROR OPENING FILE "',A,'" ON UNIT ',I5,/,
c###     &7X,'SPECIFIED FILE STATUS: ',A,/
c###     &7X,'SPECIFIED FILE FORMAT: ',A,/
c###     &7X,'SPECIFIED FILE ACCESS: ',A,/
c###     &7X,'SPECIFIED FILE ACTION: ',A,/
c###     &2X,'-- STOP EXECUTION (SGLO1BAS6OPEN)')
c###      CALL USTOP(' ')
      ierror=6
      return
C
...
c###      WRITE(IOUT,580) CMODE(MODE)(1:LENG)
  580 FORMAT(/,' MODE: ',A/)
      ...
      SUBROUTINE SGWF2BAS7ARPVAL(IUPVAL,IOUT,IUNIT,NIUNIT,ierror)
C     ******************************************************************
