...
C1------IDENTIFY PACKAGE
C      WRITE(IOUT,1)IN
C    1 FORMAT(1H0,'FHB1 -- SPECIFIED FLOW PACKAGE, VERSION 1,12/3/96',
C     &' INPUT READ FROM',I3)
C
...
C2------AUXILIARY VARIABLES.
      READ(IN,*,IOSTAT=ierror) NBDTIM,NFLW,NHED,IFHBSS,IFHBCB,NFHBX1,
     &  NFHBX2
...
C         WRITE(IOUT,4)
C 4       FORMAT(1X,'SPECIFIED FLOW AND HEAD BOUNDARY OPTION ',
C     &   'CANCELLED.',/,1X,'NO BOUNDARY CELLS WERE SPECIFIED.')
         IN=0
         RETURN
      ENDIF
      IF(NBDTIM.LT.1) THEN
C         WRITE(IOUT,6)
C 6       FORMAT(1X, 'SIMULATION ABORTING.  NOT ENOUGH TIMES ',
C     &   'SPECIFIED FOR FHB1 PACKAGE.')
         STOP
      ELSE IF(NBDTIM.EQ.1) THEN
C         WRITE(IOUT,8)
C 8       FORMAT(1X,' SPECIFIED FLOW AND HEAD VALUES WILL REMAIN ',
C     &   'CONSTANT FOR ENTIRE SIMULATION.')
      ELSE
C         WRITE(IOUT,10) NBDTIM
C 10      FORMAT(1H ,'TOTAL OF',I5,' TIMES WILL BE USED TO DEFINE ',
C     &   'VARIATIONS IN FLOW AND HEAD.')
      ENDIF
C
C4------PRINT NFLW AND NHED AND STEADY-STATE OPTION
C      WRITE(IOUT,12) NFLW
C 12   FORMAT(1H ,'FLOW WILL BE SPECIFIED AT A TOTAL OF',I5,' CELLS.')
C      WRITE(IOUT,14) NHED
C 14   FORMAT(1H ,'HEAD WILL BE SPECIFIED AT A TOTAL OF',I5,' CELLS.')
C      IF(ISS.EQ.0) THEN
C         WRITE(IOUT,15)
C 15      FORMAT(1H ,'FHB STEADY-STATE OPTION FLAG WILL BE IGNORED,'/,
C     &          1H ,'SIMULATION IS TRANSIENT.')
C      ELSE
C      IF(IFHBSS.EQ.0) THEN
C         WRITE(IOUT,16)
C 16      FORMAT(1H ,'FLOW, HEAD, AND AUX VARIABLES AT TIME=0 WILL BE ',
C     &        /,1H ,'USED IN STEADY-STATE SIMULATIONS.')
C      ELSE
C         WRITE(IOUT,18)
C 18      FORMAT(1H ,'FLOW, HEAD, AND AUX VARIABLES WILL BE ',
C     &      'INTERPOLATED',/,1H ,'IN STEADY-STATE SIMULATIONS.')
C      ENDIF
C      ENDIF
C
C5------IF CELL-BY-CELL FLOW TERMS ARE TO BE SAVED THEN PRINT UNIT #
C      IF(IFHBCB.GT.0) WRITE(IOUT,20) IFHBCB
C 20   FORMAT(1X,'CELL-BY-CELL FLOWS WILL BE RECORDED ON UNIT',I3)
C      IF(IFHBCB.LT.0) WRITE(IOUT,24)
C 24   FORMAT(1X,'CELL-BY-CELL FLOWS WILL BE PRINTED WHEN ICBCFL NOT 0')
C
C6------READ AUXILIARY VARIABLES
      IF(NFHBX1.GT.5.OR.NFHBX2.GT.5) THEN
	  ierror=20
	  goto 300
c         WRITE(IOUT,*) ' ABORTING. A MAXIMUM OF 5 AUXILIARY VARIABLES',
c     &   ' CAN BE DEFINED BY FHB.'
c         STOP
      ENDIF
C      WRITE(IOUT,26) NFHBX1
C 26   FORMAT(1X,I2,' AUXILIARY VARIABLES FOR SPECIFIED-FLOW CELLS WILL',
C     & /,'  BE DEFINED BY FHB FOR USE BY OTHER PACKAGES.')
      IF(NFHBX1.LT.1) GO TO 38
C      WRITE(IOUT,28)
C 28   FORMAT('       NAME      WEIGHTING FACTOR',/,1X,32('-'))
      DO 30 NX=1,NFHBX1
      READ(IN,'(A)') LINE
      LLOC=1
...
C      WRITE(IOUT,29) FHBXNM(NX),FHBXWT(NX)
C 29   FORMAT(1X,A16,F11.2)
      IF(FHBXWT(NX).LT.0.0.OR.FHBXWT(NX).GT.1.0) THEN
C      WRITE(IOUT,*) ' Aborting. Weights for Auxiliary variables cannot'
C      WRITE(IOUT,*) ' be less than 0.0 or greater than 1.0.'
	  ierror=20
	  goto 300
c      STOP
      ENDIF
...
c rbw begin
   38 CONTINUE
c rbw end
C 38   WRITE(IOUT,126) NFHBX2
C 126  FORMAT(1X,I2,' AUXILIARY VARIABLES FOR SPECIFIED-HEAD CELLS WILL',
C     & /,'  BE DEFINED BY FHB FOR USE BY OTHER PACKAGES.')
      IF(NFHBX2.LT.1) GO TO 200
C      WRITE(IOUT,28)
      DO 130 NX=1,NFHBX2
      READ(IN,'(A)',IOSTAT=ierror) LINE
      IF (IERROR.NE.0) goto 300
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,IOUT,IN)
      FHBXNM(5+NX)=LINE(ISTART:ISTOP)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,N,FHBXWT(5+NX),IOUT,IN)
C      WRITE(IOUT,129) FHBXNM(5+NX),FHBXWT(5+NX)
C 129  FORMAT(1X,A16,F11.2)
...
C      WRITE(IOUT,210) ISP
C 210  FORMAT(1X,I8,' ELEMENTS IN X ARRAY ARE USED BY FHB1')
      ISUM1=ISUM-1
C      WRITE(IOUT,220) ISUM1,LENX
C 220  FORMAT(1X,I8,' ELEMENTS OF X ARRAY USED OUT OF',I8)
C
C9------IF THERE ISN'T ENOUGH SPACE IN THE X ARRAY THEN PRINT
C9------A WARNING MESSAGE.
C      IF(ISUM1.GT.LENX) WRITE(IOUT,230)
C 230  FORMAT(1X,'   ***X ARRAY MUST BE DIMENSIONED LARGER***')
C10-----RETURN
300   continue
      RETURN
      END
...
C1------READ TIMES AT WHICH SPECIFIED FLOW AND HEAD VALUES WILL BE READ
      READ(IN,*,IOSTAT=ierror) IFHBUN,CNSTM,IFHBPT
      IF (IERROR.NE.0) goto 300
c      WRITE(IOUT,10) IFHBUN,CNSTM
c 10   FORMAT(1X,'TIMES FOR SPECIFIED-FLOW AND HEAD VALUES WILL BE READ',
c     & ' ON UNIT',I4,' AND',/,
c     &' MULTIPLIED BY',G12.4,'.')
      READ(IFHBUN,*,IOSTAT=ierror) (BDTIM(L),L=1,NBDTIM)
      IF (IERROR.NE.0) goto 300
      DO 12 L=1,NBDTIM
...
C2------IF DESIRED, PRINT TABLE OF TIMES
      IF(IFHBPT.GT.0) THEN
c         WRITE(IOUT,20) NBDTIM
c 20      FORMAT(1X,I5,' TIMES FOR SPECIFYING FLOWS AND HEADS:')
c         WRITE(IOUT,22) (L,L=1,NBDTIM)
c 22      FORMAT(16X,I8,4I12)
         ND=MIN0(60,NBDTIM*12)
c         WRITE(IOUT,24) (DSH1,M=1,ND)
c 24      FORMAT(17X,60A1)
c         WRITE(IOUT,26) (BDTIM(L),L=1,NBDTIM)
c 26      FORMAT(17X,5G12.4)
      ENDIF
C
...
c         WRITE(IOUT,30)
c 30      FORMAT(1X,'STARTING TIME FOR SPECIFIED FLOWS AND HEADS MUST',
c     &   ' BE ZERO. ABORTING.')
         ICHK1=1
      ENDIF
      DO 40 L=2,NBDTIM
...
c         WRITE(IOUT,32)
c 32      FORMAT(1X,'TIMES FOR SPECIFIED FLOWS MUST INCREASE.',
c     &   '  ABORTING.')
         ICHK2=1
         GO TO 42
      ENDIF
...
C4A-----READ CELL INDICIES AND SPECIFIED-FLOW RATES
      ...
      READ(IN,*,IOSTAT=ierror) IFHBUN,CNSTM,IFHBPT
      IF (IERROR.NE.0) goto 300
c      WRITE(IOUT,50) IFHBUN,CNSTM
c 50   FORMAT(/,1X,'CELL INDICIES AND SPECIFIED-FLOW RATES ',
c     & 'WILL BE READ ON UNIT',I4,'. RATES WILL',/,
c     & 1X,'BE MULTIPLIED BY',G12.4,'.')
      IF(IFHBPT.GT.0) THEN
c         WRITE(IOUT,52)
c 52      FORMAT(1X,'LAYER  ROW  COL IAUX              FLOW RATES')
         ND=MIN0(79,19+NBDTIM*12)
c         WRITE(IOUT,54) (DSH1,M=1,ND)
c 54      FORMAT(1X,78A1)
      ENDIF
      DO 59 N=1,NFLW
      READ(IFHBUN,*,IOSTAT=ierror) (IFLLOC(I,N),I=1,4),(FLWRAT(L,N),
	1  L=1,NBDTIM)
      IF (IERROR.NE.0) goto 300
      DO 56 L=1,NBDTIM
...
C4B-----AND RATES
      IF(IFHBPT.GT.0) THEN
c         WRITE(IOUT,58) (IFLLOC(I,N),I=1,4),(FLWRAT(L,N),L=1,NBDTIM)
c 58      FORMAT(1X,I4,3I5,5G12.4,/,(20X,5G12.4))
      ENDIF
 59   CONTINUE
C
C5A------READ VALUES OF AUXILIARY VARIABLES FOR SPECIFIED-FLOW CELLS
      ...
      READ(IN,*,IOSTAT=ierror) IFHBUN,CNSTM,IFHBPT
      IF (IERROR.NE.0) goto 300
c      WRITE(IOUT,61) FHBXNM(NX),IFHBUN,CNSTM
c 61   FORMAT(/,1X,A16,
c     & 'FOR SPECIFIED-FLOW CELLS WILL BE READ ON UNIT',I4,'.',/,
c     &  ' VALUES WILL BE MULTIPLIED BY',G12.4,'.')
      IF(IFHBPT.GT.0) THEN
c         WRITE(IOUT,62) FHBXNM(NX)
c 62      FORMAT(1X,'LAYER  ROW  COL IAUX  ',A16)
c         WRITE(IOUT,54) (DSH1,M=1,ND)
      ENDIF
      DO 68 N=1,NFLW
      READ(IFHBUN,*,IOSTAT=ierror) (FLWRAT(NS+L,N),L=1,NBDTIM)
      IF (IERROR.NE.0) goto 300
      DO 66 L=1,NBDTIM
...
C5B------SPECIFIED-FLOW CELL LOCATIONS
      IF(IFHBPT.GT.0) THEN
c         WRITE(IOUT,58) (IFLLOC(I,N),I=1,4),
c     &                        (FLWRAT(NS+L,N),L=1,NBDTIM)
c 67      FORMAT(1X,I4,2I6,5G12.4,/,(17X,5G12.4))
      ENDIF
 68   CONTINUE
 69   CONTINUE
C
C6------READ CELL INDICIES AND SPECIFIED-HEAD VALUES
 70   IF(NHED.LT.1) GO TO 300
      READ(IN,*,IOSTAT=ierror) IFHBUN,CNSTM,IFHBPT
      IF (IERROR.NE.0) goto 300
c      WRITE(IOUT,71) IFHBUN,CNSTM
c 71   FORMAT(/,1X,'CELL INDICIES AND SPECIFIED-HEAD VALUES ',
c     & 'WILL BE READ ON UNIT',I4,'. HEAD VALUES',/,
c     & 1X,'WILL BE MULTIPLIED BY',G12.4,'.')
      IF(IFHBPT.GT.0) THEN
c         WRITE(IOUT,72)
c 72      FORMAT(1X,'LAYER  ROW  COL IAUX             HEAD VALUES')
         ND=MIN0(79,19+NBDTIM*12)
c         WRITE(IOUT,74) (DSH1,M=1,ND)
c 74      FORMAT(1X,79A1)
      ENDIF
      DO 80 N=1,NHED
      READ(IFHBUN,*,IOSTAT=ierror) (IHDLOC(I,N),I=1,4),(SBHED(L,N),
	1  L=1,NBDTIM)
      IF (IERROR.NE.0) goto 300
      DO 75 L=1,NBDTIM
...
c         IBOUND(J,I,K)=-IABS(IBOUND(J,I,K))
c      ELSE
c         WRITE(IOUT,76) (IHDLOC(I,N),I=1,3)
c 76      FORMAT(1X,'SPECIFIED-HEAD VALUE IGNORED AT ROW',I5,', COLUMN',
c     &   I5,', AND LAYER',I5,'.')
c      ENDIF
C
...
C8------AND RATES
      IF(IFHBPT.GT.0) THEN
c         IF(IBOUND(J,I,K).NE.0) 
c     &   WRITE(IOUT,58) (IHDLOC(I,N),I=1,4),(SBHED(L,N),L=1,NBDTIM)
      ENDIF
 80   CONTINUE
C
C9A------READ VALUES OF AUXILIARY VARIABLES FOR SPECIFIED-HEAD CELLS
      ...
      READ(IN,*,IOSTAT=ierror) IFHBUN,CNSTM,IFHBPT
      IF (IERROR.NE.0) goto 300
c      WRITE(IOUT,161) FHBXNM(5+NX),IFHBUN,CNSTM
c 161  FORMAT(/,1X,A16,
c     & 'FOR SPECIFIED-HEAD CELLS WILL BE READ ON UNIT',I4,'.',/,
c     &  ' VALUES WILL BE MULTIPLIED BY',G12.4,'.')
c      IF(IFHBPT.GT.0) THEN
c         WRITE(IOUT,62) FHBXNM(5+NX)
c         WRITE(IOUT,54) (DSH1,M=1,ND)
c      ENDIF
      DO 168 N=1,NHED
      READ(IFHBUN,*,IOSTAT=ierror) (SBHED(NS+L,N),L=1,NBDTIM)
      IF (IERROR.NE.0) goto 300
      DO 166 L=1,NBDTIM
...
c      IF(IFHBPT.GT.0) THEN
c         WRITE(IOUT,58) (IHDLOC(I,N),I=1,4),
c     &                        (SBHED(NS+L,N),L=1,NBDTIM)
c      ENDIF
 168  CONTINUE
 169  CONTINUE
C
