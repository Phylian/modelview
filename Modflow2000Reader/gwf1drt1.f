...
C=======================================================================
     ...
     &                      DRTF,IDRTPB,MXDRT,IFREFM,ITERP,
     &                      IDRTFL,INAMLOC)
C
C-----VERSION 20000620 ERB
C     ******************************************************************
...
c###      IF (ITERP.EQ.1) WRITE(IOUT,500) NPDRT
...
c###          WRITE(IOUT,500) NDRTNP,MXADRT
...
c###          WRITE(IOUT,62) L,IL,IR,IC,Q
   62     FORMAT(1X,'DRAIN ',I4,'   LAYER ',I3,'   ROW ',I4,
     &       '   COL ',I4,'   RATE ',1PG15.6)
          IF (ILR.NE.0) THEN
c###            WRITE(IOUT,550) L,ILR,IRR,ICR,QIN
...
C  Check for and decode EXTERNAL and SFAC records.
      ...
      IF (LINE(ISTART:ISTOP).EQ.'EXTERNAL') THEN
        ...
        IN=I
c###        IF (ITERP.EQ.1) WRITE(IOUT,510) IN
  510   FORMAT(1X,'Reading list on unit',I4)
      ...
      ELSEIF (LINE(ISTART:ISTOP).EQ.'OPEN/CLOSE') THEN
        ...
        IN=NUNOPN
c###        IF (ITERP.EQ.1) WRITE(IOUT,520) IN,FNAME
...
C  return flow.
        ...
        IF (ITERP.EQ.1) THEN
          IF (IDRTFL.EQ.0) THEN
c###            WRITE(IOUT,570) NN,K,I,J,(DRTF(JJ,II),JJ=4,NREAD2)
  570       FORMAT(1X,I6,I7,I7,I7,14G16.4)
          ELSE
            IF (NREAD2.GE.10) THEN
c###              WRITE(IOUT,570) NN,K,I,J,(DRTF(JJ,II),JJ=4,5),
c###     &                        (DRTF(JJ,II),JJ=10,NREAD2)
            ELSE
c###              WRITE(IOUT,570) NN,K,I,J,(DRTF(JJ,II),JJ=4,5)
            ENDIF
          ENDIF
...
c###          WRITE(IOUT,*) ' ERROR: Layer number is outside of the grid'
          IERR = 1
        ENDIF
        IF (I.LT.1 .OR. I.GT.NROW) THEN
c###          WRITE(IOUT,*) ' ERROR: Row number is outside of the grid'
          IERR = 1
        ENDIF
        IF (J.LT.1 .OR. J.GT.NCOL) THEN
c###          WRITE(IOUT,*) ' ERROR: Column number is outside of the grid'
          IERR = 1
        ENDIF
        IF (IERR.NE.0) STOP
  100 CONTINUE
...
C     Check and write data related to return-flow recipient cells
      IF (IDRTFL.GT.0) THEN
c###        IF (ITERP.EQ.1) WRITE(IOUT,'(/,1X,A,/,1X,A)') LABEL2,LABEL3
        NN = 0
        DO 110 II=LSTBEG,N
          NN = NN + 1
          K = DRTF(1,II)
          ...
          JR = DRTF(8,II)
          RFP = DRTF(9,II)
c###          IF (ITERP.EQ.1) WRITE(IOUT,600) NN,K,I,J,KR,IR,JR,RFP
  600     FORMAT(1X,I6,3I7,3I7,2X,F8.6)
C
C  Check for illegal grid location
          IF (KR.NE.0) THEN
            IF (KR.LT.0 .OR. KR.GT.NLAY) THEN
c###              WRITE(IOUT,*) ' ERROR: Layer number is outside of the',
c###     &                      ' grid'
              IERR = 1
            ENDIF
...
c###              WRITE(IOUT,*) ' ERROR: Column number is outside of the',
c###     &                      ' grid'
              IERR = 1
            ENDIF
C
...
c###            WRITE(IOUT,520) NDRTCL,MXADRT
...
C  Substitute values
   ...
   40       CONTINUE
            ...
            IC=DRTF(3,II)
            IF (IDRTFL.EQ.0) THEN
c###              WRITE(IOUT,530) II,IL,IR,IC,(DRTF(JJ,II),JJ=4,NREAD)
  530         FORMAT(1X,I6,I7,I7,I7,14G16.4)
            ELSE
              IF (NREAD.GE.10) THEN
c###                WRITE(IOUT,530) II,IL,IR,IC,(DRTF(JJ,II),JJ=4,5),
c###     &                          (DRTF(JJ,II),JJ=10,NREAD)
              ELSE
c###                WRITE(IOUT,530) II,IL,IR,IC,(DRTF(JJ,II),JJ=4,5)
              ENDIF
            ENDIF
...
C
  120 CONTINUE
C
c###C     WRITE DATA RELATED TO RETURN-FLOW RECIPIENT CELLS
      IF (IDRTFL.GT.0) THEN
c###        WRITE(IOUT,'(/,1X,A,/,1X,A)') LABEL2,LABEL3
        NN = 0
        DO 140 II=NDRTCL-NLST+1,NDRTCL
          ...
          JR = DRTF(8,II)
          RFP = DRTF(9,II)
c###          WRITE(IOUT,600) NN,K,I,J,KR,IR,JR,RFP
  600     FORMAT(1X,I6,3I7,3I7,2X,F8.6)
  140   CONTINUE
      ENDIF
C
