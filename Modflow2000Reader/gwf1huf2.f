...
c###        WRITE(IOUT,17)
   17   FORMAT(/1X,'IOHUFFLWS NOT FOUND AND WILL BE SET TO BE ZERO',/)
      ENDIF
c###      IF(IOHUFFLWS.GT.0) WRITE(IOUT,18) IOHUFFLWS
...
c###   39 IF(NLAY.GT.200) THEN
c###         WRITE(IOUT,41)
c###   41    FORMAT(1X,/1X,'YOU HAVE SPECIFIED MORE THAN 200 MODEL LAYERS'/
c###     & 1X,'SPACE IS RESERVED FOR A MAXIMUM OF 200 LAYERS IN HUF ARRAYS')
c###         CALL USTOP(' ')
c###      END IF
C
C4------READ LTHUF, LAYWT.
...
c###            WRITE(IOUT,*)
c###     &          ' LAYWT is not 0 and LTHUF is 0 for layer:',K
c###            WRITE(IOUT,*) ' LAYWT must be 0 if LTHUF is 0'
c###            CALL USTOP(' ')
            ierror = 1
            return
          ELSE
...
c###        WRITE(IOUT,78) K,LTHUF(K),LAYPRN(1),LAYWT(K),LAYPRN(5)
...
C=======================================================================
     ...
     & IHGUFLG,ITERP,NHUF,NPHUF,HUFTHK,ITRSS,ierror)
C
C     ******************************************************************
...
c###         WRITE(IOUT,11)
   11    FORMAT(1X,/,1X,'WETTING CAPABILITY IS NOT ACTIVE IN ANY LAYER')
      ELSE
c###         WRITE(IOUT,12) IWDFLG
   12    FORMAT(1X,/,1X,'WETTING CAPABILITY IS ACTIVE IN',I4,' LAYERS')
         IWDFLG=1
         READ(IN,*) WETFCT,IWETIT,IHDWET
         IF(IWETIT.LE.0) IWETIT=1
c###         WRITE(IOUT,*) ' WETTING FACTOR=',WETFCT
c###         WRITE(IOUT,*) ' WETTING ITERATION INTERVAL=',IWETIT
c###         WRITE(IOUT,*) ' IHDWET=',IHDWET
      END IF
C
...
c###          WRITE(IOUT,79) HGUNAM(K),HANNAM,LAYPRN
        ELSE
c###          WRITE(IOUT,80) HGUNAM(K),HGUHANI(K),LAYPRN
        END IF
      ...
      IF(NPHUF.GT.0) THEN
        DO 20 K=1,NPHUF
          ...
          ELSE IF(PTYP.EQ.'SYTP') THEN
            NPSYTP=1
          ELSE
c###            WRITE(IOUT,*) ' Invalid parameter type for HUF Package'
c###            CALL USTOP(' ')
            ierror = 1
            return
          END IF
...
c###      WRITE(IOUT,'(/,A)') 'Reading PRINTCODE information'
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,0,N,R,IOUT,IN)
C  Find the unit name in the list
      TMPNAM=LINE(ISTART:ISTOP)
      CALL UPCASE(TMPNAM)
...
c###      WRITE(IOUT,442)
  442 FORMAT(//,'PRINTCODE FLAGS ARE SET AS FOLLOWS',/,
     ...
     &  '------------------------------------------')
      DO 480 NU=1,NHUF
c###        WRITE(IOUT,444) HGUNAM(NU),(IHGUFLG(I,NU),I=1,5)
  444 FORMAT(A10,5I6)
  480 CONTINUE
...
c======================================================================
     ...
     & ILVDA,VDHD,VDHT,IWETIT,IHDWET,WETFCT,GS,A9,ierror)
C
C     ******************************************************************
...
C Check for cells that GO DRY/REWET
     ...
     &   WETFCT,IWETIT,IHDWET,HDRY,ierror)
...
! Model Viewer code
      ...
      DO 200 J=1,NCOL
      DO 200 NK=1,NLAY
        IF(IBOUND(J,I,NK).NE.0) then

          BOTL = BOTM(J,I,NK)
		if (LTHUF(NK).eq.0) then
            TOPL = BOTM(J,I,NK-1)
		else
	      TOPL = Min(HNEW(J,I,NK),BOTM(J,I,NK-1))
		endif
	    if (TOPL.gt.BOTL) then
	      HK(J,I,NK) = HK(J,I,NK)/(TOPL-BOTL)
	    else
	      HK(J,I,NK) = 0;
		endif 
	  endif
...
c     done for model viewer
c###C
c###C-------CALCULATE SC1
c###      IF(ITRSS.NE.0 .AND. KITER.EQ.0 .AND. KPER.EQ.0 .AND.
c###     &   KSTP.EQ.0) THEN
c###C
c###C-------CHECK FOR SYTP PARAMETERS
c###        CALL SGWF1HUF2SYTP(SC1,IBOUND,NCOL,NROW,NLAY,IZON,
c###     &                     NZONAR,RMLT,NMLTAR)
c###C
c###C-------MULTIPLY BY DELR*DELC TO GET SC1
c###        CALL SGWF1HUF2SC(IBOUND,SC1,DELR,DELC,NCOL,
c###     &                   NROW,NLAY)
c###      ENDIF
c###C
c###C-----Print HUF data
c###      IF(KITER.EQ.0) THEN
c###C-----Print unit arrays depending on flags
c###        DO 200 NU=1,NHUF
c###          DO 210 IP=1,5
c###            IF(IHGUFLG(IP,NU).GT.0) THEN
c###              IF(IP.EQ.3.AND.HGUVANI(NU).GT.0) THEN
c###                IA=6
c###              ELSE
c###                IA=IP
c###              ENDIF
c###C-----Populate HUFHK as a temporary holding array
c###              DO 220 J=1,NCOL
c###                DO 220 I=1,NROW
c###                  HUFHK(NU)=0
c###                  CALL UHUF2POP(HUFHK,PTYPE(IP),NCOL,NROW,NHUF,I,J,
c###     &                   HUFTHK,IZON,NZONAR,RMLT,NMLTAR,NU,IOUT)
c###                  IF(IA.EQ.6)
c###     &            CALL UHUF2POP(HUFHK,PTYPE(IA),NCOL,NROW,NHUF,I,J,
c###     &                   HUFTHK,IZON,NZONAR,RMLT,NMLTAR,NU,IOUT)
c###C-----Transfer to HUFTMP
c###                  HUFTMP(J,I,NU)=HUFHK(NU)
c###                  IF(IA.EQ.6.AND.HUFHK(NU).EQ.0.)
c###     &              HUFTMP(J,I,NU)=HGUVANI(NU)
c###                  IF(IA.EQ.2.AND.HGUHANI(NU).GT.0..AND.HUFHK(NU).EQ.0.)
c###     &              HUFTMP(J,I,NU)=HGUHANI(NU)
c###  220         CONTINUE
c###C-----Print HUFTMP
c###              WRITE(IOUT,305) ANAME(IA),HGUNAM(NU)
c###  305         FORMAT(//A24,' FOR UNIT ',A10)
c###              CALL ULAPRWC(HUFTMP(1,1,NU),NCOL,NROW,0,IOUT,
c###     &                     IHGUFLG(IP,NU),ANAME(IA))
c###            ENDIF
c###  210     CONTINUE
c###  200   CONTINUE
c###      ENDIF
...
c======================================================================
     ...
     & HUFVK,NHUF,NU,LAMBDA,IOUT,GS,ierror)
C
C     ******************************************************************
...
c###            WRITE(IOUT,510) I,J,NU
c###            CALL USTOP(' ')
            ierror=1
            return
          ELSEIF(MULTKDEP.EQ.0.) THEN
c###            WRITE(IOUT,520) I,J,NU
c###            CALL USTOP(' ')
            ierror=1
            return
          ENDIF
...
c###         WRITE(IOUT,43) K,I,J
   ...
   50    CONTINUE
      END IF
   60 CONTINUE
c###      IF(IZFLG.EQ.1) WRITE(IOUT,500)
  500 FORMAT(/,1X
...
c###               WRITE(IOUT,151)
  151          FORMAT(1X,/1X,'CONSTANT-HEAD CELL WENT DRY',
     &          ' -- SIMULATION ABORTED')
c###                    WRITE(IOUT,*) TTOP, BBOT, THCK
c###               WRITE(IOUT,152) K,I,J,KITER,KSTP,KPER
...
c###         IF(IHDCNV.EQ.0) WRITE(IOUT,17) KITER,K,KSTP,KPER
   17    FORMAT(1X,/1X,'CELL CONVERSIONS FOR ITER.=',I3,'  LAYER=',
     &       I3,'  STEP=',I3,'  PERIOD=',I3,'   (ROW,COL)')
         IHDCNV=1
c###         WRITE(IOUT,18) (ACNVRT(L),ICNVRT(L),JCNVRT(L),L=1,NCNVRT)
...
C=======================================================================
     ...
     & IN,IOUT,ITERP,NPKDEP,IFKDEP,NROW,NCOL,GS,TOP,NHUF,ierror)
C
C     ******************************************************************
...
C-------READ GROUND SURFACE OR TRANSFER FROM BOTTOM ARRAY
      IF(IFKDEP.GT.0) THEN
c###        WRITE(IOUT,*) 'Reading ground surface'
        CALL U2DREL(GS,'GROUND SURFACE          ',NROW,NCOL,0,IN,IOUT)
      ELSE
c###        WRITE(IOUT,*) 'Transferring ground surface from TOP'
        DO 100 I=1,NROW
          DO 200 J=1,NCOL
            GS(J,I) = TOP(J,I)
  200     CONTINUE
      ...
      IF(NPKDEP.GT.0) THEN
        DO 20 K=1,NPKDEP
          ...
          IF(PTYP.EQ.'KDEP') THEN
            CONTINUE
          ELSE
c###            WRITE(IOUT,*) ' Invalid parameter type for KDEP capability'
c###            CALL USTOP(' ')
            ierror=1
            return
          END IF
...
c======================================================================
     ...
     &  NLAY,LCVDHD,LCDVDH,LCVDHT,NPLVDA,LCA9,ierror)
C
C     ******************************************************************
...
C=======================================================================
     ...
     & IN,IOUT,ITERP,NHUF,NPLVDA,NLAY,ISEN,ierror)
C
C     ******************************************************************
      ...
      IF(NPLVDA.GT.0) THEN
        DO 20 K=1,NPLVDA
          ...
          IF(PTYP.EQ.'LVDA') THEN
            CONTINUE
          ELSE
c###            WRITE(IOUT,*) ' Invalid parameter type for LVDA capability'
c###            CALL USTOP(' ')
            ierror=1
            return
          END IF
...
C     LAYERS
   ...
   30   CONTINUE
        IF(KLAYFLG.NE.0) THEN
c###          WRITE(IOUT,*) ' LVDA cannot calculate sensitivities for',
c###     & ' convertible layers!'
c###          WRITE(IOUT,*) ' STOP EXECUTION - GWF1HUF2LVDA1RPGD'
c###          CALL USTOP(' ')
          ierror=1
          return
        ENDIF
