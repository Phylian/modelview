     ...
     &  NHUF,NPHUF,LCHUFTHK,LCHKCC,ISUMI,IOHUF,LAYHDT,LCHUFTMP,ierror)
C
C     ******************************************************************
...
c###      if(NHUF.gt.0) write(iout,12) NPHUF
   12 format(1x,'Hydrogeologic-Unit Flow Package Active with ',i3,
     1  ' parameters')
c###      IF(IOHUF.gt.0) write(iout,15) IOHUF
...
c###            WRITE(IOUT,*)
c###     1          ' LAYWT is not 0 and LTHUF is 0 for layer:',K
...
C=======================================================================
     ...
     2 IHGUFLG,ITERP,NHUF,NPHUF,HUFTHK,ITRSS,ierror)
C
C     ******************************************************************
...
c###        WRITE(IOUT,*) 'Simulation is transient and no storage ',
c###     &    'parameters are defined in the HUF Package (STOP)'
	   ierror=20
	   return
c###        STOP
      ENDIF
c###      WRITE(IOUT,*) 'ITRSS',ITRSS
      IF(ITRSS.EQ.0.AND.(NPSS.NE.0.OR.NPSY.NE.0)) THEN
c###        WRITE(IOUT,*) 'Simulation is steady state and storage ',
c###     &    'parameters are defined in the HUF Package (STOP)'
	   ierror=20
	   return
c###        STOP
...
c###        WRITE(IOUT,*) 'Simulation is transient and has convertible ',
c###     &    'layers and only one storage parameter is defined in the HUF',
c###     &    ' Package (STOP)'
	   ierror=20
	   return
c###        STOP
...
C  Illegal duplicate
c###                  WRITE(IOUT,*) ' Duplicate parameter name'
                  return
c###                  STOP
              END IF
...
c###         WRITE(IOUT,36) IPCLST(1,I),LINE(IM1:IM2),LINE(IZ1:IZ2)
   36    FORMAT(1X,'               LAYER:',I3,'   MULTIPLIER:',A,
     2      '   ZONE:',A)
      ELSE
c###         WRITE(IOUT,37) LINE(IM1:IM2),LINE(IZ1:IZ2)
...
C  Find the multiplier array number
   ...
   40    CONTINUE
c###         WRITE(IOUT,'(A)') ' Multiplier array has not been defined'
                  return
c###         STOP
   45    IPCLST(2,I)=J
      END IF
...
c###            STOP
         END IF
c###         WRITE(IOUT,48) (IPCLST(J,I),J=5,IPCLST(4,I))
   ...
   50    CONTINUE
c###         WRITE(IOUT,'(A)') ' Zone array has not been defined'
                  return
c###         STOP
   55    IPCLST(3,I)=J
      END IF
...
c###              HKCC(J,I,K)=HKCC(J,I,K)/(TOP-BOT)
          ENDIF
   71   CONTINUE
      KK=K
C
...
c###        DO 11 I=1,NROW
c###        DO 11 J=1,NCOL
c###          IF(IBOUND(J,I,K).NE.0.AND.VKA(J,I,K).GT.0)  THEN
c###            CV(J,I,K)=DELR(J)*DELC(I)/VKA(J,I,K)
c###          ELSE
c###            CV(J,I,K)=0.
c###          ENDIF
c###   11    CONTINUE
...
C14-----PRINT THE FLOW FOR THE CELL IF REQUESTED.
      IF(IBD.LT.0) THEN
c###         IF(IBDLBL.EQ.0) WRITE(IOUT,899) TEXT,KPER,KSTP
  899    FORMAT(1X,/1X,A,'   PERIOD',I3,'   STEP',I3)
c###         WRITE(IOUT,900) K,I,J,RATE
...
c###        WRITE(IOUT,72) KSTP,KPER,NU,HGUNAM(NU)
   72   FORMAT('1',
...
c###        IF(IFIRST.EQ.1) WRITE(IOUT,74) IOHUF,KSTP,KPER
