C     Last change:  AWH  05JUNE 2000
     ...
     1        NRIVVL,IRIVAL,IFREFM,NPRIV,IRIVPB,NRIVNP,ierror)
C
...
C6------RETURN.
     ...
     1            RIVR,IRIVPB,MXRIVR,IFREFM,ITERP,INAMLOC)
C
...
C###      IF(ITERP.EQ.1) WRITE(IOUT,3) NPRIV
...
C6------RETURN
     ...
     1       IFREFM,NCOL,NROW,NLAY,NRIVNP,NPRIV,IRIVPB,ierror)
C
...
C3------IF THERE ARE NEW NON-PARAMETER REACHES, READ THEM.
      MXACTR=IRIVPB-1
      IF(ITMP.GT.0) THEN
         IF(NRIVNP.GT.MXACTR) THEN
C###            WRITE(IOUT,99) NRIVNP,MXACTR
...
C###            STOP
         END IF
         CALL ULSTRD(NRIVNP,RIVR,1,NRIVVL,MXRIVR,IRIVAL,IN,
...
C###         WRITE(IOUT,62) L,IL,IR,IC,RATE
